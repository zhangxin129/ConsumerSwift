...
+ (BOOL)isServiceHSCardNo:(NSString *)serviceHSCardNo{
    NSString* regex = @"^[0-9]{11}$";
    NSPredicate* pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    BOOL isMatch = [pred evaluateWithObject:serviceHSCardNo];
    if (!isMatch)
        return NO;
    
    NSString* cardNo1 = [serviceHSCardNo substringWithRange:NSMakeRange(0, 2)];
    NSString* cardNo2 = [serviceHSCardNo substringWithRange:NSMakeRange(2, 3)];
    NSString* cardNo3 = [serviceHSCardNo substringWithRange:NSMakeRange(5, 2)];
    NSString* cardNo4 = [serviceHSCardNo substringWithRange:NSMakeRange(7, 4)];
...
+ (NSString *)PointCode:(NSString *)pointCode{
    if (pointCode.length >= 14 && [pointCode hasPrefix:@"ID&"] && [GYUtils isHSCardNo:[pointCode substringWithRange:NSMakeRange(3, 11)]]) {
        return [pointCode substringWithRange:NSMakeRange(3, 11)];
    }else if (pointCode.length == 15 && [[pointCode substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"11"] && [[pointCode substringWithRange:NSMakeRange(2, 2)] isEqualToString:@"01"] && [GYUtils isHSCardNo:[pointCode substringWithRange:NSMakeRange(4, 11)]]){
        return [pointCode substringWithRange:NSMakeRange(4, 11)];
    }
}
+ (BOOL)isPointCode:(NSString *)pointCode{
 ...
 *  判断支付码
 */
+ (NSString *)consumptionCode:(NSString *)consumptionCode{
    if (consumptionCode.length == 22 && [[consumptionCode substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"21"]){
        return [consumptionCode substringWithRange:NSMakeRange(2, 11)];
    }

}
+ (BOOL)isConsumptionCode:(NSString *)consumptionCode{
    if (consumptionCode.length == 22 && [[consumptionCode substringWithRange:NSMakeRange(0, 2)] isEqualToString:@"21"]){
        return YES;
    }
}
