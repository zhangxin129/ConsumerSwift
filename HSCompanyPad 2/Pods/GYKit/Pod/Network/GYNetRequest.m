...
- (instancetype)init
{
    ...
    if (self = [super init]) {
        ...
        _networkReachable = [YYReachability reachability].reachable;
        _noShowErrorMsg = NO;
        GYKitDebugLog(@"init class:%@.", [self class]);
        _returnCodeName = @"retCode";
        _returnCodeAry = [NSMutableArray arrayWithObject:@200];
        ;
    }
    return self;
}
...
{
    if (paramDic == nil ||
        [paramDic isKindOfClass:[NSDictionary class]] == NO ||
        [paramDic count] <= 0) {
        GYKitDebugLog(@"The paramDic is invalid.");
        return;
    }

    [_HTTPHeaderFieldConfig setDictionary:paramDic];
}
...
- (void)dealloc
{
    ...
    [self cancel];
    GYKitDebugLog(@"dealloc class:%@.", [self class]);
}
