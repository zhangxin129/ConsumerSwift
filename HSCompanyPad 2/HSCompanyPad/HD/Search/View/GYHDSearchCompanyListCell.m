...
-(void)refreshWithModel:(GYHDSearchCompanyListModel *)model{
    ...
    if ([model.icon hasPrefix:@"http"]) {
        ...
        [_iconImage setImageWithURL:[NSURL URLWithString:model.icon] placeholder:[UIImage imageNamed:@"gyhd_defaultheadimg"] options:kNilOptions completion:nil];
    }else{
        
        [_iconImage setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",globalData.loginModel.picUrl,model.icon]] placeholder:[UIImage imageNamed:@"gyhd_defaultheadimg"] options:kNilOptions completion:nil];
    }
    ...
    if (model.hsCard!=nil&&model.hsCard.length>0) {
        
        _hsCardLabel.text=[NSString stringWithFormat:@"%@:%@",kLocalized(@"GYHD_Alternate_Card"),model.hsCard];
    }
