...
- (void)setModel:(GYHDChatModel *)model {
    ...
    if (model.messageImage) {
        self.chatPictureImageView.image =  model.messageImage;
    }else if ([model.messageNetWorkBasePath hasPrefix:@"http"]){
    ...
    }else if ([model.messageFileBasePath hasPrefix:@"assets-library:"]){
        NSURL *url = [NSURL URLWithString:model.messageFileBasePath];
        @weakify(self);
    ...
    }else if (model.messageFileBasePath){
...
- (void)setSessionModel:(GYHDSessionRecordModel *)sessionModel {
    ...
    if ([sessionModel.messageNetWorkBasePath hasPrefix:@"http"]){
    ...
    }else if ([sessionModel.messageFileBasePath hasPrefix:@"assets-library:"]){
        NSURL *url = [NSURL URLWithString:sessionModel.messageFileBasePath];
        @weakify(self);
        ALAssetsLibrary* assetLibrary = [[ALAssetsLibrary alloc] init];
        [assetLibrary assetForURL:url resultBlock:^(ALAsset* asset) {
            @strongify(self);
    ...
    }else if (sessionModel.messageFileBasePath){
...
{
    if ([self.delegate respondsToSelector:@selector(GYHDChatView:tapType:chatModel:)]) {
        [self.delegate GYHDChatView:self tapType:GYHDChatTapChatImage chatModel:self.model];
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(GYHDChatView:tapType:sessionModel:)]) {
        
        [self.delegate GYHDChatView:self tapType:GYHDChatTapChatImage sessionModel:self.sessionModel];
    }
}
