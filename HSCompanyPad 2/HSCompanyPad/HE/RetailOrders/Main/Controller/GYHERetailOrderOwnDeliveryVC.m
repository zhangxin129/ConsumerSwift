...
@interface GYHERetailOrderOwnDeliveryVC () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIImageView* topBackView;
@property (nonatomic, strong) UITableView* tableView;

@property (nonatomic, strong) UITextField* orderNumTextField;
@property (nonatomic, strong) UITextField* mentionCodeTextField;
...
@implementation GYHERetailOrderOwnDeliveryVC
...
- (void)viewDidLayoutSubviews
{
    ...
    self.mentionCodeTextField.customBorderColor = kGrayDDDDDD;
    self.mentionCodeTextField.customBorderType = UIViewCustomBorderTypeAll;
...
- (void)addTopView
{
    ...
    [self.topBackView addSubview:self.mentionCodeTextField];
    [self.mentionCodeTextField mas_makeConstraints:^(MASConstraintMaker* make) {
        make.left.equalTo(_orderNumTextField.mas_right).offset(kDeviceProportion(10));
        make.top.equalTo(_topBackView).offset(kDeviceProportion(11));
        make.width.equalTo(@(kDeviceProportion(83)));
        make.height.equalTo(@(kDeviceProportion(25)));
    }];
    ...
    [self.resNoOrPhoneTextField mas_makeConstraints:^(MASConstraintMaker* make) {
        make.left.equalTo(_mentionCodeTextField.mas_right).offset(kDeviceProportion(10));
        make.top.equalTo(_topBackView).offset(kDeviceProportion(11));
        make.width.equalTo(@(kDeviceProportion(157)));
        make.height.equalTo(@(kDeviceProportion(25)));
    }];
...
- (UIView*)topBackView
{
    if (!_topBackView) {
        ...
        _topBackView.multipleTouchEnabled = YES;
    }
    return _topBackView;
}
...
- (UITextField*)mentionCodeTextField
{
    if (!_mentionCodeTextField) {
        _mentionCodeTextField = [[UITextField alloc] init];
        _mentionCodeTextField.textColor = kGrayCCCCCC;
        _mentionCodeTextField.font = kFont24;
        _mentionCodeTextField.placeholder = @"自提码";
        _mentionCodeTextField.delegate = self;
        _mentionCodeTextField.keyboardType = UIKeyboardTypeNumberPad;
        _mentionCodeTextField.leftViewMode = UITextFieldViewModeAlways;
        UIImageView* leftView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 5, 5)];
        _mentionCodeTextField.leftView = leftView;
    }
    return _mentionCodeTextField;
}
