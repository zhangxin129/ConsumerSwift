...
@implementation GYNotificationHub {
...
- (void)setView:(UIView *)view andCount:(NSUInteger)startCount
{
    ...
    redCircle.backgroundColor = [UIColor colorWithRed:230/255.0f green:33/255.0f blue:41/255.0f alpha:1.0f];
    
    countLabel = [[UILabel alloc]initWithFrame:redCircle.frame];
    countLabel.userInteractionEnabled = YES;
//    add zhangx 添加 移除手势
    UITapGestureRecognizer *tap=[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapClick)];
    [countLabel addGestureRecognizer:tap];
...
-(void)tapClick{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(clearAllMessage)]) {
        
        [_delegate clearAllMessage];
    }
