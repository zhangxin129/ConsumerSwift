...
class DrawerMenuController: UIViewController ,UIGestureRecognizerDelegate {
    ...
    var minScale:CGFloat = 1                  //root 最小缩放
    ...
    var bounces:Bool = false                              //滑动边界回弹
    var springBack:Bool = false  //是否支持回弹效果 object—c 只有ios7 以上有效 swift ios7以下未知 没有测试过
    ...
    override func viewDidLoad() {
        ...
        blackCoverPan = UIPanGestureRecognizer(target: self, action: #selector(DrawerMenuController.blackCoverGesture(_:)))
    ...
    func moveViewWithGesture(sender: UIPanGestureRecognizer)
    {
        ...
        let velocity:CGPoint=movePan!.velocityInView(self.view)
        ...
        let currentPostion = movePan!.translationInView(self.view)
