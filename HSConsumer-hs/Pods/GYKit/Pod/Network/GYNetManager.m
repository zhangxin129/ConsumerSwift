...
- (void)startRequest:(GYNetRequest*)request
{
    ...
    if (request.animateObj) {
        self.requestResult = YES;
    }

    [self startAnimate:request];
...
- (void)startAnimate:(GYNetRequest*)request
{
    if (request.animateObj) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (self.requestResult) {
                [request.animateObj startAnimating];
            }
        });
    }
}

- (void)stopAnimate:(GYNetRequest*)request
{
    if (request.animateObj) {
        [request.animateObj stopAnimating];
