...
@interface GYPhotoGroupItem () <NSCopying>
@property (nonatomic, readonly) UIImage* thumbImage;
@property (nonatomic, readonly) BOOL thumbClippedToTop;
- (BOOL)shouldClipToTop:(CGSize)imageSize forView:(UIView*)view;
@end
@implementation GYPhotoGroupItem

- (UIImage*)thumbImage
{
    if ([_thumbView respondsToSelector:@selector(image)]) {
        return ((UIImageView*)_thumbView).image ? ((UIImageView*)_thumbView).image : [UIImage imageNamed:@"placeholder_image"];
    }
...
{
    GYPhotoGroupItem* item = [self.class new];
    return item;
}
@end
...
@property (nonatomic, strong) GYPhotoGroupItem* item;
...
- (void)setItem:(GYPhotoGroupItem*)item
{
    if (_item == item)
        return;
    _item = item;
...
- (void)resizeSubviewSize
{
    ...
    if (image.size.height / image.size.width > self.height / self.width ) {
        if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad) {//不是ipad才进行尺寸适配
...
@interface GYPhotoGroupView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation GYPhotoGroupView
                  ...
                  completion:(void (^)(void))completion
{
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((GYPhotoGroupItem*)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    GYPhotoGroupItem* item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion
{
    ...
    GYPhotoGroupItem* item = _groupItems[currentPage];
...
- (void)updateCellsForReuse
{
    for (YYPhotoGroupCell* cell in _cells) {
        if (cell.superview) {
            if (cell.left > _scrollView.contentOffset.x + _scrollView.width * 2 || cell.right < _scrollView.contentOffset.x - _scrollView.width) {
...
- (void)longPress
{
    ...
    [[UIActivityViewController alloc] initWithActivityItems:@[ imageItem ]
