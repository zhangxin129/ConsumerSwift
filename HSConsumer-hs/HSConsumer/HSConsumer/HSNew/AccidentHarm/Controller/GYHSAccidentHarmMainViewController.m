//
//  GYHSAccidentHarmMainViewController.m
//  HSConsumer
//
...
@interface GYHSAccidentHarmMainViewController ()<GYViewControllerDelegate,UIScrollViewDelegate,MenuTabViewDelegate>

@property (weak, nonatomic) IBOutlet UIScrollView *AccidentscrllView;
@property (strong, nonatomic)GYHSEnsureQualificationViewController *EnsureQualificationVC;//我的保障资格
@property (strong, nonatomic)GYHSApplicationSecurityViewController *ApplicationSecurityVC;//申请保障金
...
@implementation GYHSAccidentHarmMainViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    self.title = kLocalized(@"GYHS_BP_Accident_Harm_Security");
    ...
    //添加 tableview
    self.ApplicationSecurityVC = kLoadVcFromClassStringName(NSStringFromClass([GYHSApplicationSecurityViewController class]));
    self.ApplicationSecurityVC.nav = self.navigationController;
    self.EnsureQualificationVC = kLoadVcFromClassStringName(NSStringFromClass([GYHSEnsureQualificationViewController class]));
    ...
    for (int i = 0; i < _menuTitles.count; i++) {
        ...
        GYHSEnsureQualificationViewController* vc = _arrParentViews[i];
