...
@interface GYHSModifyEmailBandingViewController () <GYNetRequestDelegate>
// 验证码倒计时时间

@property (nonatomic, assign) int leftTime;
@end

@implementation GYHSModifyEmailBandingViewController {
...
- (void)viewDidLoad
{
    ...
    //国际化修改名称
    [self modifyName];
    InputOldEmail.tfRightTextField.keyboardType = UIKeyboardTypeEmailAddress;
    InputNewEmail.tfRightTextField.keyboardType = UIKeyboardTypeEmailAddress;
    InputPasswordRow.tfRightTextField.keyboardType = UIKeyboardTypeNumberPad;
}
...
- (void)netRequest:(GYNetRequest*)request didSuccessWithData:(NSDictionary*)responseObject
{
    ...
    if ([responseObject[@"retCode"] integerValue] == 200) {
        ...
        [GYAlertView showMessage:[NSString stringWithFormat:kLocalized(@"%@%@%@"), kLocalized(@"GYHS_Banding_Validation_Email_Sent%@Please_Login_Email_Complete_Binding"), InputNewEmail.tfRightTextField.text, kLocalized(@"GYHS_Binding_Login_Your_Email_Finish_Banding")]
                    confirmBlock:^{
...
- (void)modifyName
{
    ...
    btnNextStep.backgroundColor = kNavigationBarColor;
    btnNextStep.layer.cornerRadius = 15.0f;
    [btnNextStep setTitle:kLocalized(@"GYHS_Banding_ImmediatelyChange") forState:UIControlStateNormal];

    InputOldEmail.lbLeftlabel.text = kLocalized(@"GYHS_Binding_New_Email");
    InputOldEmail.tfRightTextField.placeholder = kLocalized(@"请输入新邮箱");//GYHS_Banding_Enter_New_Email
    InputNewEmail.lbLeftlabel.text = kLocalized(@"GYHS_Banding_ConfirmationEmail");
    InputNewEmail.tfRightTextField.placeholder = kLocalized(@"请再次输入新邮箱");//GYHS_Banding_EnterNewEmailAgain
    InputPasswordRow.lbLeftlabel.text = kLocalized(@"登录密码");//GYHS_Banding_Verified_Pwd
    InputPasswordRow.tfRightTextField.placeholder = kLocalized(@"请输入6位登录密码");//GYHS_Banding_Placeholder_Verified_Pwd
