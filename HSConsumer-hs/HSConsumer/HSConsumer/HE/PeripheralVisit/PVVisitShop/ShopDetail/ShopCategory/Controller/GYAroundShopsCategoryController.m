...
- (void)toSearchGoodsWithIndex:(NSInteger)index cellIndexPath:(NSIndexPath*)indexPath type:(NSInteger)type
{
    ...
    GYAroundSearchShopGoodsViewController* vcSearch = [[GYAroundSearchShopGoodsViewController alloc] init];
    vcSearch.vshopId = self.strShopID;
    vcSearch.categoryName = strcategoryName;
    vcSearch.categoryId = strCID;
    vcSearch.isNeedLoad = YES;
...
{
    if (self.marrRows.count > section && self.marrShowAll.count > section) {
...
- (void)getData
{
    ...
    GYNetRequest *request = [[GYNetRequest alloc] initWithBlock:GetVShopCategoryUrl parameters:dict requestMethod:GYNetRequestMethodGET requestSerializer:GYNetRequestSerializerHTTP respondBlock:^(NSDictionary *responseObject, NSError *error) {
        if (!error) {
