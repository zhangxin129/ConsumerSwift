...
- (void)viewDidLoad
{
    ...
    [_orderstable registerNib:[UINib nibWithNibName:NSStringFromClass([GYgreensCell class]) bundle:kDefaultBundle] forCellReuseIdentifier:@"GYgreensCellID"];
    [_orderstable registerNib:[UINib nibWithNibName:NSStringFromClass([GYDeliveryTableViewCell class]) bundle:kDefaultBundle] forCellReuseIdentifier:@"GYDeliveryTableViewCellID"];
    ...
    for (int i = 0; i < self.orderConfirModel.foodArr.count; i++) {
        ...
        [fordic setObject:@(format.price.floatValue) forKey:@"price"];
...
- (void)initView
{
    _orderstable = ({
        ...
        [tableview registerNib:[UINib nibWithNibName:NSStringFromClass([GYAddresseeCell class]) bundle:nil] forCellReuseIdentifier:@"CELLTITLT"];
        tableview;
    });
...
- (void)baseView
{
    ...
    baselabel.text = [NSString stringWithFormat:@"%0.2f", [self.orderConfirModel.totalAmount floatValue] + [self.orderConfirModel.sendPrice floatValue]];
    ...
    pvnumlabel.text = [NSString stringWithFormat:@"%0.2f", [self.orderConfirModel.totalPv floatValue]];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == rowCount - 1) { ////配送费
            ...
            deliveryCell.labDeliveryMoney.text = [NSString stringWithFormat:@"%0.2f", [_orderConfirModel.sendPrice floatValue]];
