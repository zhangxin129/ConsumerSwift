...
@interface GYEPSaleAfterApplyForChangeGoodsViewController1 () <UITableViewDataSource, UITableViewDelegate, CellForReturnGoodsCellDelegate, UITextViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, GYNetRequestDelegate,GYGetPhotoViewDelegate>
...
@property (weak, nonatomic) IBOutlet UILabel* uploadTipLabel; //上传提示
@property (weak, nonatomic) IBOutlet NSLayoutConstraint* diatanceTop; //上传图片功能修改
@property (weak, nonatomic) IBOutlet NSLayoutConstraint* tableViewDistanceTop;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint* distabceLeft;
...
@property (assign, nonatomic) CGFloat itemWH; //collectionVie的cell宽高
@property (assign, nonatomic) CGFloat margin; //collectionVie的cell间距
...
@property (nonatomic, weak) UIImageView* ivup;
...
{
    if (!_tableView) {
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(_instructionView.frame.origin.x, kDefaultMarginToBounds - 10, kScreenWidth - 30, [CellForReturnGoodsCell getHeight] * self.arrDataSource.count - 1) style:UITableViewStylePlain];
...
- (void)viewDidLayoutSubviews
{
    ...
    [self.tableView addTopBorderAndBottomBorder];
...
- (void)viewDidLoad
{
    ...
    [_ivUploadPicture setImage:kLoadPng(@"gycommon_myHE_upload_img")];
...
- (void)setLabelAndButton
{
    ...
    _urlString = @"";
    self.distabceLeft.constant = 15;
    _probLabel.text = kLocalized(@"GYHE_MyHE_RefundEnter200Characters");
    _probLabel.textColor = [UIColor lightGrayColor];
    _returnInfoLab.text = kLocalized(@"GYHE_MyHE_ChangeGoodsInstruction");
    _returnInfoLab.font = [UIFont systemFontOfSize:13];
    _returnInfoLab.textColor = kCorlorFromHexcode(0x8C8C8C);
    _inputTipLabel.text = nil;
...
- (void)setHideKeyBoardAndScrollViewFrame
{
    ...
    if (self.arrDataSource.count < 1) {
        WS(weakSelf)
            [GYUtils showMessage:kLocalized(@"GYHE_MyHE_NoConductedReplacementGoods") confirm:^{
...
- (void)configCollectionView
{
    ...
    self.diatanceTop.constant = _itemWH - 95;
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(_margin, _instructionView.bounds.size.height + _instructionView.frame.origin.y + 15 + (self.arrDataSource.count - 1) * 80, kScreenWidth - 28, _itemWH * 2 + 12) collectionViewLayout:layout];
...
{
    self.tableViewDistanceTop.constant = 80 * (self.arrDataSource.count - 1);
...
- (void)didFailUploadImg:(NSError*)error withTag:(int)index
{
    ...
    _urlString = @"";
    [_ivup setImage:kLoadPng(@"gycommon_myHE_upload_img")];
    [GYGIFHUD dismiss];
}
...
- (IBAction)btnApplyForClick:(id)sender
{
    ...
    if(!_orderDetailsIDs || [_orderDetailsIDs isEqualToString:@""]) {
        ...
        [GYUtils showMessage:kLocalized(@"GYHE_MyHE_PleaseWriteChangeGoodsInstruction")];
        return;
        
    }else if ([GYUtils checkStringInvalid:_urlString]){
        [GYUtils showMessage:kLocalized(@"GYHE_MyHE_PleaseUpdatePhoto")];
        return;
    }
    [self applyFor];
}
...
- (void)updateAmountAndItemIdsBySelect
{
    ...
    for (int i = 0; i < self.arrDataSource.count; i++) {
        ...
        if (cell.selected) {
            totalAmount += kSaftToDouble(self.arrDataSource[i][@"price"]);
            totalAmountPv += kSaftToDouble(self.arrDataSource[i][@"pv"]);
...
- (void)netRequest:(GYNetRequest*)request didSuccessWithData:(NSDictionary*)responseObject
{
    ...
    if (kSaftToNSInteger(dic[@"retCode"]) == kEasyPurchaseRequestSucceedCode) {
