...
#import "GYHSGetGoodViewController.h" //收货地址
...
{
    _userInfoHeader = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([GYMyHsHeader class]) owner:self options:nil] firstObject];
...
- (void)createDataArray
{
    ...
    NSArray* arr5 = @[
        ...
        @{ kKeyPowerIcon : @"gyhe_cell_food_address",
            ...
            kKeyNextVcName : NSStringFromClass([GYHSGetGoodViewController class]) //送餐地址
        },
    ];
    NSArray* arr6 = @[
...
- (void)pushVC:(id)sender animated:(BOOL)ani
{
    if (!sender)
        return;
    if ([sender isKindOfClass:[GYHSGetGoodViewController class]]) {
        GYHSGetGoodViewController* vc = (GYHSGetGoodViewController*)sender;
        vc.isFood = YES;
