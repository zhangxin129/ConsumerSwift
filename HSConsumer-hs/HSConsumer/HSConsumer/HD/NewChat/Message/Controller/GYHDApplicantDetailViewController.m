...
@interface GYHDApplicantDetailViewController ()<GYHDSDKDelegate>
/**好友头像View*/
...
- (void)viewDidLoad
{
    ...
    [agreeButton setBackgroundImage:[UIImage imageNamed:@"gyhd_argee_btn_normal"] forState:UIControlStateNormal];
    ...
    [agreeButton setTitle:[GYUtils localizedStringWithKey:@"GYHD_Friend_agree1"] forState:UIControlStateNormal];
    [agreeButton setTitle:[GYUtils localizedStringWithKey:@"GYHD_ADD_success"] forState:UIControlStateSelected];
...
//        make.top.left.bottom.right.equalTo(extraInformationContLabel);
    ...
    if (hushengCard.length>15) {
        
            friendHushengLabel.text = [NSString stringWithFormat:@"%@: %@",[GYUtils localizedStringWithKey:@"GYHD_hushengCard"],[[GYHDMessageCenter sharedInstance] segmentationHuShengCardWithCard:[hushengCard substringWithRange:NSMakeRange(4, 11)]]];
    }else {
...
{
    NSLog(@"分类");
    GYHDTagViewController *tagViewController = [[GYHDTagViewController alloc] init];
    
    
    if (self.model.applicantID.length > 15) {
        if ([self.model.applicantID rangeOfString:@"nc"].location != NSNotFound)  {
             tagViewController.friendCustID = [self.model.applicantID substringWithRange:NSMakeRange(5, 11)];
        }else {
             tagViewController.friendCustID = [self.model.applicantID substringWithRange:NSMakeRange(4, 11)];
        }
...
//    NSDictionary* infoDict = [GYUtils stringToDictionary:self.model.applicantBody];
    ...
    insideDict[@"reqInfo"] = [NSString stringWithFormat:@"%@ 同意添加你为好友",globalData.loginModel.custId];
    WS(weakSelf);
    [[GYHDSDK sharedInstance] verifyFriendWithFriendID:self.model.applicantID agree:0 content:[GYUtils dictionaryToString:insideDict]];
    [GYHDSDK sharedInstance].receiveMsgBlock = ^(PBGeneratedMessage *message){
        if ([message isKindOfClass:[VerifyFriendRsp class]]) {
            ...
            if (rsp.resultCode == ResultCodeNoError){
                weakSelf.model.applicantUserStatus = 0;
                ...
                selectDict[GYHDDataBaseCenterPushMessagePlantFromID] =weakSelf.model.applicantID;
                ...
                if (teamID) {
                    ...
                    sendDict[@"userId"] =  dict[@"Friend_ID"];
                    sendDict[@"friendId"] = [weakSelf.model.applicantID substringFromIndex:2];
                ...
                UIWindow *windows = [UIApplication sharedApplication].keyWindow;
                [windows makeToast:@"添加失败" duration:1.0f position:CSToastPositionCenter];
            }

        }
        
    };
    //    [GYHDSDK sharedInstance].delegate = self;
//    NSDictionary* infoDict = [GYUtils stringToDictionary:self.model.applicantBody];
