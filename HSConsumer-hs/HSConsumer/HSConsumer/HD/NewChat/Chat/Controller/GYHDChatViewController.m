...
- (void)viewDidLoad
{
    ...
    [chatTableView registerClass:[GYHDChatAdvisoryCell class] forCellReuseIdentifier:@"GYHDChatAdvisoryCellID"];
    ...
    if (myDict[@"Friend_ID"]) {
        self.sendMessageID = [NSString stringWithFormat:@"m_%@", myDict[@"Friend_ID"]];
    }
    else {

        if (globalData.loginModel.cardHolder) {
            self.sendMessageID = [NSString stringWithFormat:@"m_c_%@", globalData.loginModel.custId];
        }
        else {
            self.sendMessageID = [NSString stringWithFormat:@"m_nc_%@", globalData.loginModel.custId];
        }
    }
    //. 添加通知
    [self addObserver];
...
//    self.recvMessageID = [NSString stringWithFormat:@"%@", custIDString];
    
    self.messageCard = [custIDString substringWithRange:NSMakeRange(0, 19)];
    ...
    if (selectArray.count) {
        ...
        messageDict[GYHDDataBaseCenterMessageFromID] = [NSString stringWithFormat:@"%@", custIDString];
        messageDict[GYHDDataBaseCenterMessageToID] = [NSString stringWithFormat:@"m_%@", myDict[@"Friend_ID"]];
        ...
        messageDict[GYHDDataBaseCenterMessageSendState] = @(GYHDDataBaseCenterMessageSendStateSending);
        ...
        bodyDict[@"msg_note"] = friendDict[GYHDDataBaseCenterFriendName];
        bodyDict[@"msg_content"] =contString;
        bodyDict[@"msg_type"] = @"2";
        bodyDict[@"msg_icon"] = friendDict[GYHDDataBaseCenterFriendIcon] ;
...
//        NSString* saveString = @"";
    ...
    if (companyInformationDict[@"orders"] && [companyInformationDict[@"orders"] isKindOfClass:[NSDictionary class]]) {
        ...
        messageDict[GYHDDataBaseCenterMessageToID] = [NSString stringWithFormat:@"%@", custIDString];
        messageDict[GYHDDataBaseCenterMessageFromID] = [NSString stringWithFormat:@"m_%@", myDict[@"Friend_ID"]];
        long long t = ([[NSDate date] timeIntervalSince1970]) * 1000;
        messageDict[GYHDDataBaseCenterMessageID] = [NSString stringWithFormat:@"%lld", t];
...
- (void)setMessageCard:(NSString*)messageCard
{
    ...
    if ([dict[@"Friend_MessageType"] isEqualToString:@"Friend"]) {
        self.recvMessageID = [NSString stringWithFormat:@"m_%@", dict[@"Friend_ID"]];
        ...
        if (self.sessionID == nil) {
            WS(weakSelf)
            [[GYHDSDK sharedInstance] creatPersonToCompanySessionReqWithSelfCustid:globalData.loginModel.custId entCustid:dict[GYHDDataBaseCenterFriendCustID] companyname:dict[GYHDDataBaseCenterFriendName] companylogo:dict[GYHDDataBaseCenterFriendIcon]];
            [GYHDSDK sharedInstance].sessionBlock = ^(CreateP2CSessionRsp *sessionRsp) {
                
                if (sessionRsp.code == ResultCodeErrorP2CKefuOffline) {

                    weakSelf.recvMessageID = dict[GYHDDataBaseCenterFriendCustID];
                    weakSelf.sessionID = sessionRsp.session.sessionid;
                }else if (sessionRsp.code == ResultCodeNoError) {
                    
                    weakSelf.recvMessageID = sessionRsp.session.kefuid;
                    weakSelf.sessionID = sessionRsp.session.sessionid;
                    
                    dispatch_async(dispatch_get_main_queue(), ^{
                        UIButton* rightButton = [UIButton buttonWithType:UIButtonTypeCustom];
                        [rightButton addTarget:self action:@selector(stopAdvisory:) forControlEvents:UIControlEventTouchUpInside];
                        [rightButton setImage:[UIImage imageNamed:@"gyhd_chat_stop_Advisory"] forState:UIControlStateNormal];
                        [rightButton setTitle:@"咨询结束" forState:UIControlStateNormal];
                        rightButton.titleLabel.font = [UIFont systemFontOfSize:9.0f];
                        [rightButton setImageEdgeInsets:UIEdgeInsetsMake(-10, 0, 0, -35)];
                        [rightButton setTitleEdgeInsets:UIEdgeInsetsMake(0, -20, -25, 0)];
                        [rightButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
                        rightButton.frame = CGRectMake(0, 0, 40, 40);
                        weakSelf.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:rightButton];
                    });
...
- (void)stopAdvisory:(UIButton *)button {
    WS(weakSelf);
    NSDictionary* myDict = [[GYHDMessageCenter sharedInstance] selectMyInfo];
    NSDictionary* dict = [[GYHDMessageCenter sharedInstance] selectFriendBaseWithCardString:self.messageCard];
    [[GYHDSDK sharedInstance] closePersonToCompanySessionWithUserid:self.sendMessageID entid:self.messageCard sessionid:self.sessionID kefuid:self.recvMessageID consumerid:self.sendMessageID consumername:myDict[@"Friend_Name"] consumerhead:myDict[@"Friend_Icon"] companyname:dict[@"Friend_Name"] companylogo:dict[@"Friend_Icon"]];
    [GYHDSDK sharedInstance].receiveMsgBlock =  ^(PBGeneratedMessage *message) {
        if ([message isKindOfClass:[CloseP2CSessionRsp class]]) {
...
//            [self loadChat];
            ...
            GYHDPopView *topView = [[GYHDPopView alloc] initWithChlidView:deleteTeamView];
            deleteTeamView.block = ^(NSString *message) {
                [[GYHDSDK sharedInstance] deleteFriendWithFriendID:weakSelf.recvMessageID];
                [GYHDSDK sharedInstance].receiveMsgBlock = ^(PBGeneratedMessage *message) {
                    NSLog(@"%@",message);
                    if ([message isKindOfClass:[DelFriendRsp class]]) {
                        DelFriendRsp *rsp = (DelFriendRsp *)message;
            ...
            GYHDPopView *topView = [[GYHDPopView alloc] initWithChlidView:deleteTeamView];
            deleteTeamView.block = ^(NSString *message) {
                [[GYHDSDK sharedInstance] shieldFriendWithFriendID:weakSelf.recvMessageID];
                [GYHDSDK sharedInstance].receiveMsgBlock = ^(PBGeneratedMessage *message) {
                    NSLog(@"%@",message);
                    if ([message isKindOfClass:[ShieldFriendRsp class]]) {
                        [weakSelf.navigationController popViewControllerAnimated:YES];
                    }
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    case GYHDDataBaseCenterMessageChatVideo:
        ...
        case GYHDDataBaseCenterMessageAdvisory:
        {
            GYHDChatAdvisoryCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GYHDChatAdvisoryCellID" forIndexPath:indexPath];
            cell.chatModel = chatModel;
            baseCell = cell;
        }
            break;
...
- (CGFloat)tableView:(UITableView*)tableView heightForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    case GYHDDataBaseCenterMessageChatVideo:
        ...
        break;
        case GYHDDataBaseCenterMessageAdvisory:
            return [tableView fd_heightForCellWithIdentifier:@"GYHDChatAdvisoryCellID" configuration:^(GYHDChatAdvisoryCell* cell) {
                cell.chatModel = self.chatArrayM[indexPath.row];
            }];
            break;
    default:
...
- (NSDictionary*)sendWithMessageType:(GYHDDataBaseCenterMessageType)type saveDictString:(NSString*)dictString bodyString:(NSString*)bodyString
{
    ...
    messageDict[GYHDDataBaseCenterMessageSessionID] = self.sessionID;
    NSDate* date = [NSDate date];
    NSDateFormatter* formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSString* dateString = [formatter stringFromDate:date];
...
- (void)GYHDChatView:(UIView*)view tapType:(GYHDChatTapType)type chatModel:(GYHDNewChatModel*)chatModel
{
    ...
    case GYHDChatTapChatImage: {
        ...
        if (DataDict[@"originalName"]) {
            ...
            url = [NSURL URLWithString:[[GYHDMessageCenter sharedInstance] AdditionCheckWithUrlString:bodyDict[@"msg_content"]]];
        }
    ...
    case GYHDChatTapChatVideo: {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:mp4Path]) {
            ...
            [[GYHDMessageCenter sharedInstance] downloadDataWithUrlString:[[GYHDMessageCenter sharedInstance] AdditionCheckWithUrlString:bodyDict[@"msg_content"]] RequetResult:^(NSDictionary* resultDict) {
...
- (void)sendMessageWithModel:(GYHDNewChatModel*)chatModel
{
    ...
    messageDict[GYHDDataBaseCenterMessageIsRead] = @(0);
    messageDict[GYHDDataBaseCenterMessageSendState] = @(GYHDDataBaseCenterMessageSendStateSending);
    chatModel.chatSendState = GYHDDataBaseCenterMessageSendStateSending;
    ...
    [[GYHDMessageCenter sharedInstance] updateMessageStateWithMessageID:chatModel.chatMessageID State:GYHDDataBaseCenterMessageSendStateSending];
...
//    NSDictionary* dataDict = companyInformationDict[@"data"];
//    NSString* custIDString = dataDict[@"messageCard"];
//    weakSelf.navigationItem.title = dataDict[@"vShopName"];
//    weakSelf.shopID = dataDict[@"vShopId"];
//    weakSelf.messageCard = [custIDString substringWithRange:NSMakeRange(0, 19)];
//    NSString *resNo = [weakSelf.messageCard substringToIndex:11];
//    [[GYHDSDK sharedInstance] creatPersonToCompanySessionReqWithSelfCustid:globalData.loginModel.custId entCustid:dataDict[@"messageCard"] companyname:dataDict[@"vShopName"] companylogo:dataDict[@"logo"]];
//    [GYHDSDK sharedInstance].sessionBlock = ^(CreateP2CSessionRsp *sessionRsp) {
//        weakSelf.recvMessageID = sessionRsp.session.kefuid;
//        weakSelf.sessionID = sessionRsp.session.sessionid;
//        //        // 更新SessionID
//        //        [[GYHDMessageCenter sharedInstance] setMessageSessionID:weakSelf.sessionID resNO:resNo];
//        // 更新企业信息
//        NSMutableDictionary* selectDict = [NSMutableDictionary dictionary];
//        
//        selectDict[GYHDDataBaseCenterFriendResourceID] = resNo;
//        NSArray* selectArray = [[GYHDMessageCenter sharedInstance] selectInfoWithDict:selectDict TableName:GYHDDataBaseCenterFriendTableName];
//        if (selectArray.count) {
//            NSMutableDictionary* conditionDict = [NSMutableDictionary dictionary];
//            conditionDict[GYHDDataBaseCenterFriendResourceID] = resNo;
//            NSMutableDictionary* updateDict = [NSMutableDictionary dictionary];
//            updateDict[GYHDDataBaseCenterFriendName] = dataDict[@"vShopName"];
//            updateDict[GYHDDataBaseCenterFriendIcon] = dataDict[@"logo"];
//            updateDict[GYHDDataBaseCenterFriendFriendID] = weakSelf.recvMessageID;
//            
//            [[GYHDMessageCenter sharedInstance] updateInfoWithDict:updateDict conditionDict:conditionDict TableName:GYHDDataBaseCenterFriendTableName];
//        }
//        else {
//            NSMutableDictionary* friendDict = [NSMutableDictionary dictionary];
//            friendDict[GYHDDataBaseCenterFriendFriendID] = weakSelf.recvMessageID;
//            friendDict[GYHDDataBaseCenterFriendCustID] = weakSelf.messageCard;
//            friendDict[GYHDDataBaseCenterFriendResourceID] = resNo;
//            friendDict[GYHDDataBaseCenterFriendName] = dataDict[@"vShopName"];
//            friendDict[GYHDDataBaseCenterFriendIcon] = dataDict[@"logo"];
//            friendDict[GYHDDataBaseCenterFriendUsetType] = @"e";
//            friendDict[GYHDDataBaseCenterFriendMessageType] = GYHDDataBaseCenterTemporaryBusiness;
//            friendDict[GYHDDataBaseCenterFriendMessageTop] = @"-1";
//            friendDict[GYHDDataBaseCenterFriendInfoTeamID] = weakSelf.shopID;
//            friendDict[GYHDDataBaseCenterFriendBasic] = [GYUtils dictionaryToString:companyInformationDict];
//            friendDict[GYHDDataBaseCenterFriendDetailed] = [GYUtils dictionaryToString:companyInformationDict];
//            [[GYHDMessageCenter sharedInstance] insertInfoWithDict:friendDict TableName:GYHDDataBaseCenterFriendTableName];
//            
