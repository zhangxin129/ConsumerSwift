...
- (void)setup {
    ...
    self.chatbackgroundView.image = [UIImage imageNamed:@"gyhd_chat_self_back"];
    ...
    [self.showView addGestureRecognizer:longTap];
    ...
    [self.chatStateButton setImage:[UIImage imageNamed:@"gyhd_failure"] forState:UIControlStateNormal];
    ...
    [self.showView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(weakSelf.chatbackgroundView).offset(-2);
        make.top.left.equalTo(weakSelf.chatbackgroundView).offset(2);
        make.right.equalTo(weakSelf.chatbackgroundView).offset(-10);
    }];
...
- (void)chatPictureImageViewLongTap:(UILongPressGestureRecognizer*)longTap
{
    if (longTap.state == UIGestureRecognizerStateBegan) {
        ...
        [menu setTargetRect:self.showView.bounds inView:self.showView];
