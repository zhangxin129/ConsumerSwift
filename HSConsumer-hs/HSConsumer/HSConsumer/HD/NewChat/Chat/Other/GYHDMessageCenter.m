...
NSString* const GYHDDataBaseCenterMessageSessionID = @"MSG_SessionID";
...
NSString* const GYHDDataBaseCenterUserSetingSessionID = @"User_SessionID";
...
- (void)setDeviceToken:(NSString *)deviceToken {
    _deviceToken = [NSString stringWithFormat:@"%d_%@",kIMConsumer,deviceToken];
}
...
- (void)ReceiveMessageModel:(GYHDReceiveMessageModel *)model {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        case GYHDDataBaseCenterMessageChatAudio:        //音频消息
            {
                ...
                [self.netWorkTool downloadDataWithUrlString:[[GYHDMessageCenter sharedInstance] AdditionCheckWithUrlString:bodyDict[@"msg_content"]] filePath:filePath];
        ...
        if ([model.messageType isEqualToString:@"20"]) {
            if ([bodyDict[@"msg_content"] isEqualToString:@""]) {
                return ;
            }
        }
...
//        NSMutableDictionary *messageDict = [NSMutableDictionary dictionary];
////        NSString* const GYHDDataBaseCenterMessageTableName = @"T_MESSAGE";
//        messageDict[GYHDDataBaseCenterMessageID] = model.sendtime;
//        messageDict[GYHDDataBaseCenterMessageFromID] = model.fromid ;
//        messageDict[GYHDDataBaseCenterMessageToID] = model.toid;
//        messageDict[GYHDDataBaseCenterMessageSessionID] = model.sessionid;
//
//        if ([model.fromid rangeOfString:@"_c_"].location != NSNotFound) {
//            messageDict[GYHDDataBaseCenterMessageFriendType] = @"c";
//        } else {
//            messageDict[GYHDDataBaseCenterMessageFriendType] = @"e";
//        }
//        messageDict[GYHDDataBaseCenterMessageBody] = model.content;
//        messageDict[GYHDDataBaseCenterMessageCode] = @(GYHDDataBaseCenterMessageTypeChat);
//        NSDate *sendData = [[NSDate alloc] initWithTimeIntervalSince1970:model.sendtime.longLongValue / 1000];
//        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
//        [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
//        NSString *sendDateString = [formatter stringFromDate:sendData];
//        NSString *recvDateString = [formatter stringFromDate:[NSDate date]];
...
//        NSRegularExpression* regex = [[NSRegularExpression alloc] initWithPattern:pattern options:0 error:nil];
//        // 2.测试字符串
//        NSArray* results = [regex matchesInString:model.fromid options:0 range:NSMakeRange(0, model.fromid.length)];
//        // 3.遍历结果
//        NSTextCheckingResult* result = [results firstObject];
//        messageDict[GYHDDataBaseCenterMessageCard] = [model.fromid substringWithRange:result.range];
//        messageDict[GYHDDataBaseCenterMessageData] = @"MSG_DataString";
//        NSDictionary *bodyDict = [GYUtils stringToDictionary:model.content];
//        switch ([bodyDict[@"msg_code"] integerValue]) {
//        case GYHDDataBaseCenterMessageChatText:         //文本消息
//        case GYHDDataBaseCenterMessageChatPicture:      //图片消息
//        case GYHDDataBaseCenterMessageChatMap:
...
- (void)receivePushMsg:(GYHDPushMsgModel *)model {
    NSLog(@"xxx%@",model.fromid);
    
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSDate *sendData = [[NSDate alloc] initWithTimeIntervalSince1970:model.pushtime.longLongValue / 1000];;
            NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
            [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
            NSString *sendDateString = [formatter stringFromDate:sendData];
            NSString *recvDateString = [formatter stringFromDate:[NSDate date]];
            NSDictionary *contDict = [GYUtils stringToDictionary:model.content];
            NSMutableDictionary *pushDict = [NSMutableDictionary dictionary];
            pushDict[GYHDDataBaseCenterPushMessageID] = model.msgid;
            pushDict[GYHDDataBaseCenterPushMessagePlantFromID] = model.fromid;
            pushDict[GYHDDataBaseCenterPushMessageToID] = model.toid;
            pushDict[GYHDDataBaseCenterPushMessageFromID] = model.fromid;
            pushDict[GYHDDataBaseCenterPushMessageContent] = contDict[@"reqInfo"];
            pushDict[GYHDDataBaseCenterPushMessageBody] = model.content;
            pushDict[GYHDDataBaseCenterPushMessageSendTime] = sendDateString;
            pushDict[GYHDDataBaseCenterPushMessageRevieTime] = recvDateString;
            pushDict[GYHDDataBaseCenterPushMessageIsRead] = @(0) ;  //0 已读 1未读
            pushDict[GYHDDataBaseCenterPushMessageData] = @"-1";
            pushDict[GYHDDataBaseCenterPushMessageUnreadLocation] = @"-1";
            pushDict[GYHDDataBaseCenterPushMessageCode] =  [NSString stringWithFormat:@"%ld",model.msgtype.integerValue];
            if ([model.msgtype isEqualToString:@"add"]) {
                pushDict[GYHDDataBaseCenterPushMessageCode] = [NSString stringWithFormat:@"%ld",GYHDProtobufMessage04101];
                NSMutableDictionary *deleteDict = [NSMutableDictionary dictionary];
                deleteDict[GYHDDataBaseCenterPushMessageCode] = pushDict[GYHDDataBaseCenterPushMessageCode];
                deleteDict[GYHDDataBaseCenterPushMessagePlantFromID] = model.fromid;
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:deleteDict TableName:GYHDDataBaseCenterPushMessageTableName];
            }else if ([model.msgtype isEqualToString:@"del"]) {
                pushDict[GYHDDataBaseCenterPushMessageCode] = [NSString stringWithFormat:@"%ld",GYHDProtobufMessage04104];
                NSMutableDictionary *deleteDict = [NSMutableDictionary dictionary];
                deleteDict[GYHDDataBaseCenterPushMessageCode] = pushDict[GYHDDataBaseCenterPushMessageCode];
                deleteDict[GYHDDataBaseCenterPushMessagePlantFromID] = model.fromid;
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:deleteDict TableName:GYHDDataBaseCenterPushMessageTableName];
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:@{GYHDDataBaseCenterMessageFromID:model.fromid} TableName:GYHDDataBaseCenterMessageTableName];
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:@{GYHDDataBaseCenterMessageToID:model.fromid} TableName:GYHDDataBaseCenterMessageTableName];
                ...
                frienddeletedict[@"toID"] =  pushDict[GYHDDataBaseCenterPushMessagePlantFromID] ;
                [[GYHDMessageCenter sharedInstance] postDataBaseChangeNotificationWithDict:frienddeletedict];
                
            }else if ([model.msgtype isEqualToString:@"agree"]) {
                pushDict[GYHDDataBaseCenterPushMessageCode] = [NSString stringWithFormat:@"%ld",GYHDProtobufMessage04102];
                NSMutableDictionary *deleteDict = [NSMutableDictionary dictionary];
                deleteDict[GYHDDataBaseCenterPushMessageCode] = pushDict[GYHDDataBaseCenterPushMessageCode];
                deleteDict[GYHDDataBaseCenterPushMessagePlantFromID] = model.fromid;
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:deleteDict TableName:GYHDDataBaseCenterPushMessageTableName];
                ...
                insertMessageDict[GYHDDataBaseCenterMessageFromID] = model.fromid;
                insertMessageDict[GYHDDataBaseCenterMessageToID] = model.toid;
                insertMessageDict[GYHDDataBaseCenterMessageContent] = [NSString stringWithFormat:@"您和%@已经成为好友，现在可以聊天了噢", contDict[@"msg_note"]];
                ...
                friendDict[GYHDDataBaseCenterFriendName] = contDict[@"msg_note"];
                friendDict[GYHDDataBaseCenterFriendIcon] = contDict[@"msg_icon"];
                ...
                condDict[GYHDDataBaseCenterPushMessageCode] = @([pushDict[GYHDDataBaseCenterPushMessageCode] integerValue]);
            ...
            }else if ([model.msgtype isEqualToString:@"refuse"]) {
                pushDict[GYHDDataBaseCenterPushMessageCode] = [NSString stringWithFormat:@"%ld",GYHDProtobufMessage04103];
                NSMutableDictionary *deleteDict = [NSMutableDictionary dictionary];
                deleteDict[GYHDDataBaseCenterPushMessageCode] = pushDict[GYHDDataBaseCenterPushMessageCode];
                deleteDict[GYHDDataBaseCenterPushMessagePlantFromID] = model.fromid;
                [[GYHDMessageCenter sharedInstance] deleteInfoWithDict:deleteDict TableName:GYHDDataBaseCenterPushMessageTableName];
            }else if ([model.msgtype isEqualToString:@"01030"] ||
                      [model.msgtype isEqualToString:@"01031"] ||
                      [model.msgtype isEqualToString:@"01032"] ||
                      [model.msgtype isEqualToString:@"01033"] ||
                      [model.msgtype isEqualToString:@"01034"]) {
                NSMutableDictionary *dict = [NSMutableDictionary dictionary];
                dict[@"code"] = model.msgtype;
                dict[@"content"] = model.content;

                if ([self.delegate respondsToSelector:@selector(receiveCompanyQRWithDict:)]) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self.delegate receiveCompanyQRWithDict:dict];
                    });
                }
                return ;
            }
                ...
                NSString *sendDateString = [NSString stringWithFormat:@"%@-%@-%@ %@:%@:%@",
                                            [model.pushtime substringWithRange:NSMakeRange(0, 4)],
                                            [model.pushtime substringWithRange:NSMakeRange(4, 2)],
                                            [model.pushtime substringWithRange:NSMakeRange(6, 2)],
                                            [model.pushtime substringWithRange:NSMakeRange(8, 2)],
                                            [model.pushtime substringWithRange:NSMakeRange(10, 2)],
                                            [model.pushtime substringWithRange:NSMakeRange(12, 2)]];
                pushDict[GYHDDataBaseCenterPushMessageSendTime] = sendDateString;
                NSDictionary *subContDict = nil;
                if ([contDict[@"content"] isKindOfClass:[NSString class]]) {
                   subContDict  = [GYUtils stringToDictionary:contDict[@"content"]];
                }else if ([contDict[@"content"] isKindOfClass:[NSDictionary class]]){
                    subContDict = contDict[@"content"];
                }
              
                pushDict[GYHDDataBaseCenterPushMessageContent] = subContDict[@"msg_content"];
                pushDict[GYHDDataBaseCenterPushMessageBody] = [GYUtils dictionaryToString:subContDict];
                pushDict[GYHDDataBaseCenterPushMessageIsRead] = @(1) ;  //0 已读 1未读
                if ([model.msgtype isEqualToString:@"01009"]) {
                    pushDict[GYHDDataBaseCenterPushMessageContent] = subContDict[@"summary"];
                }else {
                   NSDictionary *subContDict2 = [GYUtils stringToDictionary:subContDict[@"msg_content"]];
                    pushDict[GYHDDataBaseCenterPushMessageContent] = subContDict2[@"summary"];
                }
            }

            [[GYHDMessageCenter sharedInstance] insertInfoWithDict:pushDict TableName:GYHDDataBaseCenterPushMessageTableName];
            
            
            [[GYHDMessageCenter sharedInstance] clearMessageHidenWithCustID:pushDict[GYHDDataBaseCenterPushMessageCode] ];
            ...
            if (![[UIApplication sharedApplication] applicationState] == UIApplicationStateActive){
                ...
                localNotification.alertBody= pushDict[GYHDDataBaseCenterMessageContent] ;
        ...
        case GYHDDataBaseCenterMessageChatMap:
            {
                ...
                if (dict[GYHDDataBaseCenterMessageSessionID]) {
                    NSString *custID = dict[GYHDDataBaseCenterMessageToID];
                    if ([custID rangeOfString:@"e_"].location != NSNotFound) {
                        [[GYHDSDK sharedInstance] sendMessageToKefuWithSessionid:dict[GYHDDataBaseCenterMessageSessionID] guid:dict[GYHDDataBaseCenterMessageID] toid:dict[GYHDDataBaseCenterMessageToID] content:dict[GYHDDataBaseCenterMessageBody]];
                    }else {
                        [[GYHDSDK sharedInstance] LeaveMessageToCompanyWithFromid:dict[GYHDDataBaseCenterMessageFromID] entid:dict[GYHDDataBaseCenterMessageToID] content:dict[GYHDDataBaseCenterMessageBody] guid:dict[GYHDDataBaseCenterMessageID] sessionid:dict[GYHDDataBaseCenterMessageSessionID]];
                    }
                    

                }else {
                    [[GYHDSDK sharedInstance] sendMessageToFriendID:dict[GYHDDataBaseCenterMessageToID] guid:dict[GYHDDataBaseCenterMessageID]  content:dict[GYHDDataBaseCenterMessageBody]];
                }
...
//                        }
                        if (dict[GYHDDataBaseCenterMessageSessionID]) {
                            ...
                            [[GYHDSDK sharedInstance] sendMessageToFriendID:dict[GYHDDataBaseCenterMessageToID] guid:dict[GYHDDataBaseCenterMessageID]  content:sendDict[GYHDDataBaseCenterMessageBody]];
                        }
                    } else {
                        [[GYHDMessageCenter sharedInstance] updateMessageStateWithMessageID:dict[GYHDDataBaseCenterMessageID] State:GYHDDataBaseCenterMessageSendStateFailure];
                    }
...
- (void)MovieFriendToTagWithDict:(NSDictionary *)dict RequetResult:(RequetResultWithDict)handler {
    [self.netWorkTool MovieFriendtoTagWithDict:dict RequetResult:handler];
}
...
- (NSDictionary*)searchFriendWithCustId:(NSString*)custID RequetResult:(RequetResultWithDict)handler
{
    if (handler) {
        [self.netWorkTool searchFriendWithCustId:custID RequetResult:^(NSDictionary* resultDict) {
            if (!resultDict || !resultDict[@"custId"]) {
                return ;
            }
            NSMutableDictionary *friendDict = [NSMutableDictionary dictionary];
...
- (void)ConcernShopUrlWithDict:(NSDictionary*)dict RequetResult:(RequetResultWithDict)handler{
    [self.netWorkTool ConcernShopUrlWithDict:dict RequetResult:handler];
}
/**取消关注*/
...
- (void)getConsumerOrCompanyInfoWithDict:(NSDictionary *)dict RequetResult:(RequetResultWithDict)handler {
    [self.netWorkTool getConsumerOrCompanyInfoWithDict:dict RequetResult:handler];
}
- (void)GetVShopShortlyInfoUrlWithResourcesNo:(NSString *)string RequetResult:(RequetResultWithDict)handler {
    [self.netWorkTool GetVShopShortlyInfoUrlWithResourcesNo:string RequetResult:handler];
}
///**加好友*/
//- (void)addFriendWithFriendID:(NSString *)friendID verify:(NSString *)verify {
//    [[GYHDSDK sharedInstance] addFriendWithFriendID:friendID verify:verify];
//}
///**同意好友*/
//- (void)verifyFriendWithFriendID:(NSString *)friendID agree:(int)agree content:(NSString *)content {
//    [[GYHDSDK sharedInstance] verifyFriendWithFriendID:friendID agree:agree content:content];
//}
- (NSString *)AdditionCheckWithUrlString:(NSString *)imageString {
    NSString *urlString = [NSString stringWithFormat:@"%@&custId=%@&loginToken=%@&channelType=4",imageString,globalData.loginModel.custId,globalData.loginModel.token];
    return urlString;
}
/**更新企业SessionID*/
- (BOOL)setMessageSessionID:(NSString *)sessionID resNO:(NSString *)resNO {
   return  [[GYHDDataBaseCenter sharedInstance] setMessageSessionID:sessionID resNO:resNO];
}
- (void)getUserByPhoneContactsWithDict:(NSDictionary *)dict RequetResult:(RequetResultWithDict)handler {
    return [self.netWorkTool getUserByPhoneContactsWithDict:dict RequetResult:handler];
}
@end
// 互生消息
// 订单消息
