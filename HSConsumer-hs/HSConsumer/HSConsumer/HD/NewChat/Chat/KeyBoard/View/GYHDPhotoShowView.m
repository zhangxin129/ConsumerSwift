...
- (void)setup
{
    ...
    [navbutton setTitle:[GYUtils localizedStringWithKey:@"GYHD_Chat_photo"] forState:UIControlStateNormal];
    [navbutton setImage:[UIImage imageNamed:@"gyhd_nav_leftView_back"] forState:UIControlStateNormal];
...
- (void)sendButtonClick
{
    if ([self.delegate respondsToSelector:@selector(GYHDPhotoShowView:SendImageData:)]) {
        ...
        if (self.photoImageView.image.size.width > 1080 || self.photoImageView.image.size.height > 1080) {
            image = [GYUtils imageCompressForWidth:self.photoImageView.image targetWidth:1080];
