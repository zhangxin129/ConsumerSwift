...
- (void)setup
{
    ...
    [photoSelectButton setImage:[UIImage imageNamed:@"gyhd_photo_choose_normal"] forState:UIControlStateNormal];
    [photoSelectButton setImage:[UIImage imageNamed:@"gyhd_photo_choose_select"] forState:UIControlStateSelected];
...
- (void)photoButtonClick:(UIButton*)button
{
    ...
    if (button.selected) {
        if (selectImageCount > 9 - 1) {
            self.photoModel.photoSelectStates = button.selected = !button.selected;
            [GYUtils showToast: [GYUtils localizedStringWithKey:@"GYHD_less_than_9"] duration:1.0f position:CSToastPositionCenter];
        }
        else {
            selectImageCount++;
        }
