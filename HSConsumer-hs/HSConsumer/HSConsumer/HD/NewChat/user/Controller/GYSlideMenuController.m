...
@interface GYSlideMenuController () <UIGestureRecognizerDelegate> {
    UIView* baseView;
    UIView* currentView;
...
@implementation GYSlideMenuController

- (instancetype)init
{

    if (self = [super init]) {
        _leftViewShowWidth = 0.8 * SCREEN_WIDTH;
        _rightViewShowWidth = 0.66 * SCREEN_WIDTH;
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    //修改登录密码成功后，不侧滑
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hideSideViewController) name:@"hideSlideMenuNotification" object:nil];
    baseView = self.view;
