...
@property (weak, nonatomic) IBOutlet UILabel *totalTagLabel;//合计标签
...
@property (weak, nonatomic) IBOutlet UILabel* transportationFeeTagLabel; //运费标签
@property (weak, nonatomic) IBOutlet UILabel *transFeeNumberLabel;
@property (weak, nonatomic) IBOutlet UILabel* consumePVLabel;//积分数
@property (weak, nonatomic) IBOutlet UILabel *couponFeeNumLabel;//抵扣金额
...
- (void)viewDidLoad
{
    ...
    [self.tabView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHESCConfirmOrderTableViewCell class]) bundle:nil] forCellReuseIdentifier:kConfirmOrderCell];
    [self.tabView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHESCOrderSectionFooterView class]) bundle:nil] forHeaderFooterViewReuseIdentifier:kOrderSectionFooterCell];
    [self.tabView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHESCOrderSectionHeaderView class]) bundle:nil] forHeaderFooterViewReuseIdentifier:kOrderSectionHeaderCell];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    GYHESCOrderModel* orderModel = self.dataSourceArray[indexPath.section];
    GYHESCCartListModel* listModel = orderModel.modelArray[indexPath.row];
...
- (void)pushToDistributionWayWithSection:(NSInteger)section
{
    ...
    vc.distributionBlock = ^(GYHESCDistributionTypeModel* typeModel) {
        ...
        if ([orderModel.deliveryType isEqualToString:@"1"]) {//是快递支付
            if ([typeModel.moneyString doubleValue] == 0) {
                orderModel.sendMoney = kLocalized(@"免邮");
...
- (NSMutableArray*)buildPaymentJson
{
    ...
    for (GYHESCOrderModel* orderModel in self.dataSourceArray) {
        ...
        payModel.sendWay = [NSString stringWithFormat:@"%@ ¥%.2lf", orderModel.sendWay, [orderModel.sendMoney doubleValue]];
        ...
        payModel.postAge = [NSString stringWithFormat:@"%.2lf",[orderModel.sendMoney doubleValue]];
...
- (void)basicSettings
{
    ...
    self.tabView.rowHeight = 122.0f;

    self.totalTagLabel.text = [NSString stringWithFormat:@"%@:",kLocalized(@"合计")];
    self.realNumberTagLabel.text = [NSString stringWithFormat:@"%@:",kLocalized(@"实付")];//HE_SC_OrderRealMoney
    self.transportationFeeTagLabel.text = [NSString stringWithFormat:@"%@:",kLocalized(@"运费")];
...
- (void)setValueForBottomView
{
    ...
    for (GYHESCOrderModel* orderModel in self.dataSourceArray) {
        ...
        //    discountPrice = 0;
        //}
        transFee += [orderModel.sendMoney doubleValue];
        totalMoney += [orderModel.totalMoney doubleValue] + [orderModel.sendMoney doubleValue];
        realMoney += [orderModel.totalMoney doubleValue] + [orderModel.sendMoney doubleValue] - discountPrice;
        totalPv += [orderModel.totalPv doubleValue];
        couponFee += discountPrice;
    } //计算实付金额与消费积分
    
    self.realMoney = [NSString stringWithFormat:@"%.2lf", realMoney];
    
    self.totalNumberLabel.text = [NSString stringWithFormat:@"%.2lf", totalMoney];
    self.transFeeNumberLabel.text = [NSString stringWithFormat:@"%.2lf", transFee];
    self.realNumerLabel.text = self.realMoney;
    self.consumePVLabel.text = [NSString stringWithFormat:@"%.2lf", totalPv];
    if (couponFee <= 0) {
        self.couponFeeNumLabel.text = [NSString stringWithFormat:@"%.2lf", couponFee];
    } else {
        self.couponFeeNumLabel.text = [NSString stringWithFormat:@"-%.2lf", couponFee];
    }
}
...
- (void)addDataForDataSourceWithDiscountModel
{
    for (NSInteger i = 0; i < self.dataSourceArray.count; i++) {
        ...
        if (kSaftToDouble(expressModel.expressFee) == 0) {
            orderModel.sendMoney = kLocalized(@"免邮");
            orderModel.coinIconWidth = 0;
        } else {
            orderModel.sendMoney = [NSString stringWithFormat:@"%.2lf", kSaftToDouble(expressModel.expressFee)];
            orderModel.coinIconWidth = 16.0f;
        }
...
//                }
            ...
            [GYUtils showMessage:kLocalized(@"HE_SC_IconPayCDSuccess") confirm:^{
...
{
    if (!_headerView) {
        _headerView = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([GYHESCOrderTableHeaderView class]) owner:self options:nil] firstObject];
...
{
    if (!_footerView) {
        _footerView = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([GYHESCOrderTableFooterView class]) owner:self options:nil] firstObject];
