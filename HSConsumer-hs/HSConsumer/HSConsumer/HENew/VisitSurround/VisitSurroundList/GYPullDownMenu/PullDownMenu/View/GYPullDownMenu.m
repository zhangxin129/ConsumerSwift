...
// 更新下拉菜单标题通知名称
NSString * const GYUpdateMenuTitleNote = @"GYUpdateMenuTitleNote";

@interface GYPullDownMenu()
//下拉菜单所有按钮
...
//下拉菜单底部view
@property(nonatomic, strong)GYBgViewCover *coverView;
//下拉底部view
...
-(void)setup
{
    ...
    _observer = [[NSNotificationCenter defaultCenter]addObserverForName:GYUpdateMenuTitleNote object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        // 获取列
...
- (void)reload
{
    ...
    if (![self.dataSource respondsToSelector:@selector(numberOfColsInMenu:)]) {
        @throw [NSException exceptionWithName:@"GYError" reason:@"未实现（numberOfColsInMenu:）" userInfo:nil];
    }
    // 判断有没有实现pullDownMenu:buttonForColAtIndex:
    if (![self.dataSource respondsToSelector:@selector(pullDownMenu:buttonForColAtIndex:)]) {
        @throw [NSException exceptionWithName:@"GYError" reason:@"pullDownMenu:buttonForColAtIndex:）" userInfo:nil];
    }
    // 判断每一列控制器的方法是否实现
    if (![self.dataSource respondsToSelector:@selector(pullDownMenu:viewControllerForColAtIndex:)]) {
        @throw [NSException exceptionWithName:@"GYError" reason:@"pullDownMenu:viewControllerForColAtIndex:这个方法未实现）" userInfo:nil];
        return;
    }
    // 判断每一列控制器的方法是否实现
    if (![self.dataSource respondsToSelector:@selector(pullDownMenu:heightForColAtIndex:)]) {
        @throw [NSException exceptionWithName:@"GYError" reason:@"pullDownMenu:heightForColAtIndex:这个方法未实现）" userInfo:nil];
        return;
    }
    // 获取有多少列
    ...
    for (NSInteger col = 0; col < cols; col++) {
        ...
        if (menuButton == nil) {
            @throw [NSException exceptionWithName:@"GYError" reason:@"pullDownMenu:buttonForColAtIndex:这个方法不能返回空的按钮）" userInfo:nil];
            return;
        }
...
- (UIView *)coverView
{
    if (_coverView == nil) {
        ...
        _coverView = [[GYBgViewCover alloc] initWithFrame:CGRectMake(coverX, coverY, coverW, coverH)];
