...
typedef NS_ENUM(NSUInteger, IMLoginState) {
    ...
    kIMLoginStateOtherLogin, //登录验证失败，可以提示检查用户和密码
...
typedef void (^receiveMessageBlock)(PBGeneratedMessage *message); //命令回调
@protocol GYHDSDKDelegate <NSObject>
- (void)protoBufDidReceiveMessage:(PBGeneratedMessage *)msg;    //接收到消息
@optional
...
/**接收到被别人加我为好友的消息*/
- (void)didReceiveAddedFriendChatMsg:(ChatMsg*)chatMsg;
/**接收到被别人删除我的消息*/
- (void)didReceiveDeletedFriendChatMsg:(ChatMsg*)chatMsg;
/**接收到对方同意加我为好友的消息*/
- (void)didReceiveAgreeFriendChatMsg:(ChatMsg*)chatMsg;
/**接收到对方拒绝加我为好友的消息*/
- (void)didReceiveRefuseFriendChatMsg:(ChatMsg*)chatMsg;
/**消息回调*/
- (void)didReceiveMessage:(PBGeneratedMessage *)message;
@end
...
@property (nonatomic, weak) id<GYHDSDKDelegate> delegate;
/**
 *  登录回调的block
 */
...
@property (nonatomic, copy)receiveMessageBlock receiveMsgBlock;
//GYHDSDK的单例方法
