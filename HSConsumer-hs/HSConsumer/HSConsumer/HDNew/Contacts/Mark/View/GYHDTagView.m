...
@end
@class GYHDTextField;
@protocol GYHDTextFieldDelegate <UITextFieldDelegate>

- (void)textFieldDidDeleteBackwardEditing:(GYHDTextField *)textField;

@end

@interface GYHDTextField : UITextField
@property(nonatomic, weak)id<GYHDTextFieldDelegate> delegate;
@end
@implementation GYHDTextField

- (void)deleteBackward {
    if ([self.delegate respondsToSelector:@selector(textFieldDidDeleteBackwardEditing:)]) {
        [self.delegate textFieldDidDeleteBackwardEditing:self];
    }
...
@interface GYHDTagView() <GYHDTextFieldDelegate>
...
- (void)commonInit
{
    ...
    _inputBgColor = [UIColor whiteColor];//kColorRGB(0xffffff);
    _inputPlaceHolderTextColor = [UIColor colorWithRed:207/255.0f green:207/255.0f blue:207/255.0f alpha:1];//kColorRGB(0xcccccc);
    _inputTextColor =[UIColor blackColor];// kColorRGB(0x000000);
    _inputBorderColor = [UIColor colorWithRed:207/255.0f green:207/255.0f blue:207/255.0f alpha:1];// kColorRGB(0xfafafa);
    _bgColor =[UIColor whiteColor]; //kColorRGB(0xffffff);
    _textColor = [UIColor blackColor];// kColorRGB(0xffae00);
    _borderColor = [UIColor colorWithRed:207/255.0f green:207/255.0f blue:207/255.0f alpha:1];//kColorRGB(0xffae00);
    _selBgColor = [UIColor whiteColor];//kColorRGB(0xffae00);
    _selTextColor = [UIColor redColor];//kColorRGB(0xffffff);
    _selBorderColor = [UIColor colorWithRed:235/255.0f green:153/255.0f blue:149/255.0f alpha:1];//kColorRGB(0xffae00);
    
    _viewMaxHeight = 130;
    self.backgroundColor = [UIColor whiteColor];// kColorRGB(0xffffff);
    ...
    {
        // 默认的标签
        UITextField* tf = [[GYHDTextField alloc] initWithFrame:CGRectMake(0, 0, _tagWidht, _tagHeight)];
...
- (NSMutableArray *)tagStrings {
      ...
      switch (_type) {
        case GYHDTagView_Type_Edit: {
            return _tagStrings;
        }
            break;
        case GYHDTagView_Type_Display: {
//            return nil;
            return _tagStrings;
        }
            break;
        case GYHDTagView_Type_Selected: {
...
-(void)layoutTagviews {
    ...
    _tfInput.hidden=(_type!=GYHDTagView_Type_Edit);
    if (_type==GYHDTagView_Type_Edit) {
...
- (void)addTagToLast:(NSString *)tag {
    ...
    if (result.count == 0) {
        ...
        [_svContainer scrollRectToVisible:CGRectMake(_svContainer.left, _svContainer.contentSize.height - 50,  [UIScreen mainScreen].bounds.size.width, 50) animated:YES];
...
- (void)textFieldDidDeleteBackwardEditing:(GYHDTextField *)textField {
    if ([textField.text isEqualToString:@""]) {
        if (textField.text.length == 0) {
            TTCheckBoxButton *tagButton =  _tagButtons.lastObject;
            if (tagButton.selected) {
                _editingTagIndex = _tagStrings.count-1;
...
- (void)setType:(GYHDTagView_Type)type {
