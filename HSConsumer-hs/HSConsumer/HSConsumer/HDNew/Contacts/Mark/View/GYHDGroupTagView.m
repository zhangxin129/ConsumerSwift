...
@interface GYHDTagViewCheckBoxButton :UIButton
...
@implementation GYHDTagViewCheckBoxButton
...
@implementation GYHDGroupTagView
{
...
- (void)layoutTags {
    ...
    for (int i=0; i<_tagButtons.count; i++) {
        GYHDTagViewCheckBoxButton* tagButton=_tagButtons[i];
...
// 添加标签按钮
- (GYHDTagViewCheckBoxButton *)tagButtonWithTag:(NSString *)tag
{
    GYHDTagViewCheckBoxButton *tagBtn = [[GYHDTagViewCheckBoxButton alloc] init];
 ...
 */
- (void)handlerTagButtonEvent:(GYHDTagViewCheckBoxButton*)sender
{
...
- (void)addTagToLast:(NSString *)tag{
    ...
    if (result.count == 0)
    {
        ...
        GYHDTagViewCheckBoxButton* tagButton=[self tagButtonWithTag:tag];
...
- (void)setDeleteString:(NSString *)deleteString {
    ...
    for (int i = 0; i < self.tagStrings.count; i ++) {
        if ([deleteString isEqualToString:self.tagStrings[i]]) {
            GYHDTagViewCheckBoxButton *button = self.tagButtons[i];
            button.selected = NO;
            return;
        }
    }
