...
@interface GYHDFriendDetailViewController () <UITableViewDelegate, UITableViewDataSource,GYHDFriendDetailCellDelegate>
...
- (void)moreSelect:(UIButton*)button
{
    ...
    NSArray *modeArray = self.friendDetailArrayM[0];
    GYHDFriendDetailModel* friendDetailModel = modeArray[1];
            ...
            deleteTeamView.block = ^(NSString *message) {
                ...
                if (message) {
                    ...
                    [[GYHDMessageCenter sharedInstance] MovieFriendWithDict:dict RequetResult:^(NSDictionary *resultDict) {
                        ...
                        frienddeletedict[@"toID"] = friendDict[@"accountId"];
                        [[GYHDMessageCenter sharedInstance] postDataBaseChangeNotificationWithDict:frienddeletedict];
                        NSArray *modeArray = weakSelf.friendDetailArrayM[0];
                        GYHDFriendDetailModel* friendDetailModel = modeArray[1];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    cell.delegate = self;
    NSArray *modeArray = self.friendDetailArrayM[indexPath.section];
    cell.friendDetailModel = modeArray[indexPath.row];
    return cell;
}
...
{
    NSArray *modeArray = self.friendDetailArrayM[indexPath.section];
    GYHDFriendDetailModel* friendDetailModel = modeArray[indexPath.row];
...
- (void)cellDidEndEdit:(NSString *)string {
    [[GYHDSDK sharedInstance] modifyFriendWithFriendID:[NSString stringWithFormat:@"m_c_%@",self.FriendCustID] remark:string];
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if ([friendDetailModel.userInfoName isEqualToString: [GYUtils localizedStringWithKey:@"GYHD_classification"]]) {
        ...
        tagViewController.friendCustID = self.FriendCustID;
...
//    teamNameModel.userInfo = friendDict[@""];
    ...
    [oneArray addObject:teamNameModel];
    [oneArray addObject:remarkModel];
    ...
    [friendArray addObject:oneArray];
    [friendArray addObject:twoArray];
    self.friendDetailArrayM = friendArray;
//    self.friendDetailArrayM = twoArray;
    ...
    [self.friendDetalTableView reloadData];
}
