//
//  GYHDRealNameAuthConfirmViewController.m
//  HSConsumer
//
...
@interface GYHDRealNameAuthConfirmViewController ()<GYHSButtonCellDelegate,UITableViewDataSource,UITableViewDelegate,GYHDNextRandomCodeBtnDelegate,GYHDUploadDocumentDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,GYGetPhotoViewDelegate,UITextFieldDelegate>
...
@implementation GYHDRealNameAuthConfirmViewController
...
- (void)loadDataFromNetwork
{
    ...
    GYNetRequest *request =[[GYNetRequest alloc] initWithBlock:urlString parameters:allParas requestMethod:GYNetRequestMethodPOST requestSerializer:GYNetRequestSerializerJSON respondBlock:^(NSDictionary *responseObject, NSError *error) {
        ...
        [GYUtils showMessage:dic[@"msg"] confirm:^{
            if ([_confirmDelegate respondsToSelector:@selector(confirmRealNameAuth)]) {
                [self.confirmDelegate confirmRealNameAuth];
            }
        }];
    }];
...
- (UIView*)tableView:(UITableView*)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section == 0) {
        ...
        lb.text = kLocalized(@"GYHD_RealnameRegister_Upload_Documents");
    }else if (section == 1){
