//
//  GYHDImportantChangePassportViewController.m
//  HSConsumer
//
...
@interface GYHDImportantChangePassportViewController ()<GYNetRequestDelegate,UITableViewDataSource,UITableViewDelegate,GYHSPostscriptCertificationCellDelegate,GYHSRealNameRegistrationCellDelegate,GYDatePikerDelegate,selectNationalityDelegate,GYHSButtonCellDelegate>
...
@implementation GYHDImportantChangePassportViewController
...
- (void)loadDataFromNetwork
{
    ...
    GYNetRequest *request = [[GYNetRequest alloc] initWithBlock:kHSGetsearchRealNameAuthInfo parameters:dict requestMethod:GYNetRequestMethodGET requestSerializer:GYNetRequestSerializerJSON respondBlock:^(NSDictionary *responseObject, NSError *error) {
        ...
        [self.passportArray[0][6] setObject:kSaftToNSString(dic[@"birthPlace"]) forKey:@"Value"];
        [self.passportArray[0][7] setObject:kSaftToNSString(dic[@"issuePlace"]) forKey:@"Value"];
        [self.passportArray[0][8] setObject:kSaftToNSString(dic[@"issuingOrg"]) forKey:@"Value"];
...
- (void)chooseSelectButton:(NSIndexPath*)indexPath
{
    if (indexPath.row ==1) {
        ...
        CGPoint point = CGPointMake(130, rect.origin.y+64);
...
{
    NSMutableDictionary *dic = self.passportArray[1][0];
    [dic setObject:value forKey:@"Value"];
...
-(void)queryCustomerInfo
{
    ...
    GYNetRequest *request = [[GYNetRequest alloc] initWithBlock:CustomerInfoUrl parameters:dict requestMethod:GYNetRequestMethodGET requestSerializer:GYNetRequestSerializerJSON respondBlock:^(NSDictionary *responseObject, NSError *error) {
        ...
        if ([_changePassportDelegate respondsToSelector:@selector(importantChangePassport:olddic:changeItem:)]) {
            [self.changePassportDelegate importantChangePassport:self.dicParams olddic:self.olddicParams changeItem:self.changeItemArr];
        }
