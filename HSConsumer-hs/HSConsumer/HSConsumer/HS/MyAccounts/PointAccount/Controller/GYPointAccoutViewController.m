...
{
    [super viewDidLoad];
    [self.pointTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSAccountHeaderCell class]) bundle:Nil] forCellReuseIdentifier:@"GYHSAccountHeaderCell"];
    [self.pointTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSCellTypeImagelabel class]) bundle:Nil] forCellReuseIdentifier:@"GYHSCellTypeImagelabel"];
    [self.pointTableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSTableViewWarmCell class]) bundle:nil] forCellReuseIdentifier:@"GYHSTableViewWarmCell"];
    //控制器背景色
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (indexPath.section == 1 && indexPath.row == 0) {
        GYHSPointInvestmentViewController* HSBVC = [[GYHSPointInvestmentViewController alloc] init];
        HSBVC.title = kLocalized(@"GYHS_MyAccounts_points_to_hsCoin");
        HSBVC.accountOperationType = GYHSPointHSBType;
        ...
        HSBVC.accountOperationType = GYHSPointInvestmentType;
       // HSBVC.integral = kSaftToNSString(self.requestDic[@"data"][@"canUsePoints"]);
        [self.navigationController pushViewController:HSBVC animated:YES];
    }
