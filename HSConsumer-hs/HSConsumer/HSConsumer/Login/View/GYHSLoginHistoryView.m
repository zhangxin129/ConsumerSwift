...
- (instancetype)initWithView:(UIView*)view dataArray:(NSArray*)dataArray
{
    ...
    CGRect frame = CGRectMake(0, 0, view.frame.size.width, view.frame.size.height - 64 - 48);
...
- (void)show
{
    ...
    [UIView animateWithDuration:0 animations:^{
        ...
        self.tableView.frame = CGRectMake(0, kGYHSLoginHistoryViewTableViewY, self.frame.size.width, [self tableViewHeight]);
    }];
}
...
- (CGFloat)tableViewHeight
{
    ...
    CGFloat tmpHeight = self.frame.size.height - kGYHSLoginHistoryViewBorder - kGYHSLoginHistoryViewTableViewY - 48;

    // 互生内嵌调整高度
    if (self.pageType == GYHSLoginVCPageTypeHS) {
        tmpHeight = tmpHeight - kGYHSLoginHistoryViewTableViewY;
    }
    else if (self.popType == GYHSLoginVCShowPopView) {
        // 325 为弹出框的高度
        tmpHeight = 325 - kGYHSLoginHistoryViewBorder - kGYHSLoginHistoryViewTableViewY * 2;
    }
...
{
    if (_tableView == nil) {
        CGRect frame = CGRectMake(0, kGYHSLoginHistoryViewTableViewY, self.frame.size.width, [self tableViewHeight]);
        _tableView = [[UITableView alloc] initWithFrame:frame style:UITableViewStylePlain];
        _tableView.delegate = self;
        _tableView.dataSource = self;
        _tableView.backgroundColor = [UIColor whiteColor];
        _tableView.scrollEnabled = YES;
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSLoginHistoryTableCell class]) bundle:nil] forCellReuseIdentifier:kGYHSLoginHistoryTableCellIdentify];
    }
