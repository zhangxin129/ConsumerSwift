...
- (void)confirmButtonAction
{
    ...
    if ([self.password isSimpleLoginPwd] == NSOrderedStringAscending || [self.pwdAgain isSimpleTransPwd] == NSOrderedStringDescending) {
        [GYUtils showMessage:kLocalized(@"GYHD_HDNew_PwdNoSetContinueNmuberPleaseReset")];
        return;
    }
    if ([self.password isSimpleLoginPwd] == NSOrderedStringSame) {
...
- (UITableView*)tableView
{
    if (_tableView == nil) {
        ...
        UILabel* titleLable = [[UILabel alloc] initWithFrame:CGRectMake(0, 9, self.view.frame.size.width - 40, 21)];
        titleLable.text = kLocalized(@"请重新设置登录密码");
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSRegisterTableCell class]) bundle:nil] forCellReuseIdentifier:kGYHSResetPasswordVC_CellIdentify];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSLoginVConfirmTableCell class]) bundle:nil] forCellReuseIdentifier:kGYHSResetPasswordVC_Cell_BtnIdentify];
    }
