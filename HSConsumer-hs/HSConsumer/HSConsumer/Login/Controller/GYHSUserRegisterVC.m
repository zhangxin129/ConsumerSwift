...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, kScreenHeight - 64) style:UITableViewStyleGrouped];
        _tableView.backgroundColor = [UIColor whiteColor];
        _tableView.scrollEnabled = NO;
        _tableView.delegate = self;
        _tableView.dataSource = self;
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSRegisterTableCell class]) bundle:nil] forCellReuseIdentifier:kGYHSUserRegisterVC_CellIdentify];
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([GYHSLoginVConfirmTableCell class]) bundle:nil] forCellReuseIdentifier:kGYHSUserRegisterVC_Cell_Button_Identify];
    }
