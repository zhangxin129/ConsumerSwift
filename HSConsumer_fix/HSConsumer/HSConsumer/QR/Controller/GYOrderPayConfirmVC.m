...
@interface GYOrderPayConfirmVC () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UPPayPluginDelegate>
...
@implementation GYOrderPayConfirmVC
#pragma mark - 懒加载
...
- (void)viewDidLoad
{
    ...
    //    UIButton* quickyPayBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    //    quickyPayBtn.frame = CGRectMake(20, CGRectGetMaxY(btn.frame) + 5, 80, 30);
    //    [quickyPayBtn setTitleColor:kCorlorFromRGBA(0, 160, 223, 1) forState:UIControlStateNormal];
    //    [quickyPayBtn setTitle:@"快捷支付" forState:UIControlStateNormal];
    //    [quickyPayBtn addTarget:self action:@selector(quickPay:) forControlEvents:UIControlEventTouchUpInside];
    //
    //    [footerView addSubview:quickyPayBtn];
    //
    //    UIView* lineView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(quickyPayBtn.frame) + 5, CGRectGetMaxY(btn.frame) + 10, 0.5, 20)];
    //    lineView.backgroundColor = kCellItemTextColor;
    //    [footerView addSubview:lineView];
    //
    //    UIButton* bankPayBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    //    bankPayBtn.frame = CGRectMake(CGRectGetMaxX(lineView.frame) + 5, CGRectGetMaxY(btn.frame) + 5, 80, 30);
    //    [bankPayBtn setTitleColor:kCorlorFromRGBA(0, 160, 223, 1) forState:UIControlStateNormal];
    //    [bankPayBtn setTitle:@"网银支付" forState:UIControlStateNormal];
    //    [bankPayBtn addTarget:self action:@selector(banckPay:) forControlEvents:UIControlEventTouchUpInside];
    //    [footerView addSubview:bankPayBtn];
...
- (void)quickPay:(UIButton*)btn
{

    [self.view endEditing:YES];
    kCheckLogined;
    if (self.quickBankListArrM.count == 0) {
        [GYUtils showToast:@"您未绑定快捷支付卡，请到网页端消费者系统绑卡或者选择其它支付方式!"];
        return;
    }
    NSDateFormatter* dateFormat = [[NSDateFormatter alloc] init]; //实例化一个NSDateFormatter对象
    ...
    [Network GET:[HSReconsitutionUrl stringByAppendingString:@"/cardReader/sourceTransNo"] parameters:@{ @"entResNo" : self.model.entResNo } completion:^(id responseObject, NSError* error) {
        if (!error && [responseObject[@"retCode"] isEqualToNumber:@200]) {
            ...
            [Network Post:[HSReconsitutionUrl stringByAppendingString:@"/customer/pointBanking"]  parameters:paramDic  completion:^(id responseObject, NSError *error) {
                [GYGIFHUD dismiss];
                if (!error &&[responseObject[@"retCode"] isEqualToNumber:@200]) {
                    GYOrderQuickPayConfirmVC *confirmVC = [[GYOrderQuickPayConfirmVC alloc] init];
                    confirmVC.model = self.model;
                    confirmVC.transNo = responseObject[@"data"][@"transNo"];
                    confirmVC.quickBankListArrM = weakSelf.quickBankListArrM;
    ...
    [Network GET:[HSReconsitutionUrl stringByAppendingString:@"/cardReader/sourceTransNo"] parameters:@{ @"entResNo" : self.model.entResNo } completion:^(id responseObject, NSError* error) {
        if (!error && [responseObject[@"retCode"] isEqualToNumber:@200]) {
            ...
            [Network Post:[HSReconsitutionUrl stringByAppendingString:@"/customer/pointBanking"]  parameters:paramDic  completion:^(id responseObject, NSError *error) {
                [GYGIFHUD dismiss];
                if (!error && [responseObject[@"retCode"] isEqualToNumber:@200]) {
                    NSString *tnCode = responseObject[@"data"][@"tnTransNo"];
                    if (tnCode.length > 0) {
