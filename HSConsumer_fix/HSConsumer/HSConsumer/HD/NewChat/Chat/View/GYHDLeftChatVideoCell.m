...
- (void)setup
{
    ...
    chatVideoReadyState.image = [UIImage imageNamed:@"gyhd_chat_unread_icon"];
...
- (void)setChatModel:(GYHDNewChatModel*)chatModel
{
    ...
    [self.chatPictureImageView setImageWithURL:[NSURL URLWithString:bodyDict[@"msg_imageNail"]] placeholder:[UIImage imageNamed:@"gycommon_image_placeholder"] options:kNilOptions completion:^(UIImage* _Nullable image, NSURL* _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError* _Nullable error) {
...
//        }
    ...
    NSDictionary* saveDict = [GYUtils stringToDictionary:chatModel.chatDataString];
