...
- (NSArray*)selectFriendWithTeamID:(NSString*)teamID
{
    ...
    [self.IMdatabaseQueue inDatabase:^(FMDatabase* db) {
        ...
        if (teamID) {
            selectChatSql = [NSString stringWithFormat:@"SELECT * FROM T_FRIENDS WHERE Friend_TeamID = '%@' AND  Friend_Application != '1' AND Friend_TeamIDFriendSet !='blacklisted' ", teamID];
        } else {
