...
- (void)showLoginView:(NSInteger)dismissToBarIndex otherLogin:(BOOL)otherLogin
{
    ...
    GYHSLoginViewController* loginVc = [[GYHSLoginViewController alloc] init];
    loginVc.loginType = GYHSLoginViewControllerTypeHashsCard;

    if (![GYUtils checkStringInvalid:loginTypeKey] && [loginTypeKey isEqualToString:kGYHSLoginManagerNoCard]) {
        loginVc.loginType = GYHSLoginViewControllerTypeNohsCard;
    }

    loginVc.dismissBarIndex = dismissToBarIndex;
    loginVc.needToMainInterface = otherLogin;
    GYNavigationController* loginNV = [[GYNavigationController alloc] initWithRootViewController:loginVc];
    [kAppDelegate.window.rootViewController presentViewController:loginNV animated:YES completion:nil];
}
...
- (NSString*)userDeviceIP
{
    ...
    NSString* userIP = [ip dictionaryValue][@"data"][@"ip"];

    if ([GYUtils checkStringInvalid:userIP]) {
        userIP = [UIDevice currentDevice].ipAddressCell ? [UIDevice currentDevice].ipAddressCell : [UIDevice currentDevice].ipAddressWIFI;
    }

    DDLogDebug(@"userIP:%@", userIP);
    return kSaftToNSString(userIP);
}
