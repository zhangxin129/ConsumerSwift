...
@interface GYHealthUploadImgViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UINavigationControllerDelegate, UIImagePickerControllerDelegate, GYUploadPicDelegate,GYGetPhotoViewDelegate,GYNetRequestDelegate> {
    GYBigPic* bigPic;
}
...
@implementation GYHealthUploadImgViewController
#pragma mark 懒加载
...
- (NSMutableArray*)dataArr
{
    if (_dataArr == nil) {
        ...
        _dataArr = [NSMutableArray arrayWithObjects:
                                   ...
                                   str2,
                                   str3,
                                   kLocalized(@"GYHS_BP_Other_Attachments_Upload"), nil];
    }
    return _dataArr;
}
...
- (void)initUI
{
    ...
    [self.mycollectionView registerNib:[UINib nibWithNibName:@"GYHealthUploadImgCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"collectioncell"];
    [self.mycollectionView registerNib:[UINib nibWithNibName:@"GYCollectionFooterView" bundle:nil] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"Footer"];
}
...
- (UICollectionReusableView*)collectionView:(UICollectionView*)collectionView viewForSupplementaryElementOfKind:(NSString*)kind atIndexPath:(NSIndexPath*)indexPath
{
    if ([kind isEqualToString:UICollectionElementKindSectionFooter]) {
        GYCollectionFooterView* v = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"Footer" forIndexPath:indexPath];
        //立即申请按钮点击事件
        v.cb = ^() {
            [self btnApply];
        };
...
- (void)btnApply
{
    if ([globalData.loginModel.creType isEqualToString:kCertypeBusinessLicence]) {
        [GYUtils showMessage:kLocalized(@"GYHS_BP_Registration_Certificate_Type_Enterprise_Not_Enjoy_Alternate_Health_Benefits") confirm:nil];
        return;
    }

    if (![self isAllSnowUpload]) {
        [GYUtils showMessage:kLocalized(@"GYHS_BP_TheBelt*Photo_MustBe_Uploaded") confirm:nil];
        return;
    }
