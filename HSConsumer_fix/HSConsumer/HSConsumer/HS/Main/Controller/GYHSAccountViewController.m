...
- (void)viewDidLoad
{
    ...
    self.hasHSCardAry = @[ @{ kKeyAccIcon : @"hs_cell_img_points_account",
        ...
        @{ kKeyAccIcon : @"hs_cell_img_HSC_to_cash_acc",
            kKeyAccName : kLocalized(@"GYHS_Main_HSB_Accounts"),
            kKeyAccBal : [NSNumber numberWithDouble:globalData.user.HSDToCashAccBal],
    ...
    self.NoHSCardAry = @[ @{ kKeyAccIcon : @"hs_cell_img_HSC_to_cash_acc",
                             kKeyAccName : kLocalized(@"GYHS_Main_HSB_Accounts"),
                             kKeyAccBal : [NSNumber numberWithDouble:globalData.user.HSDToCashAccBal],
                             kKeyNextVcName : NSStringFromClass([GYHSDToCashAccoutViewController class]) },
        @{ kKeyAccIcon : @"hs_cell_img_cash_account",
    ...
    //注册自定义cell，并绑定方法
    [self.tableView registerNib:[UINib nibWithNibName:@"CellTypeImagelabel" bundle:kDefaultBundle] forCellReuseIdentifier:kCellMyAccountCellIdentifier];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (section == 0) {
        cellUserInfo = [[[NSBundle mainBundle] loadNibNamed:@"CellUserInfo" owner:self options:nil] lastObject];
...
- (void)click
{
    ...
    if (globalData.loginModel.cardHolder) {
        UIViewController* vc = kLoadVcFromClassStringName(@"GYRealNameRegistrationViewController");
        [self pushVC:vc animated:YES];
    } else {
    ...
    //pos机订单支付(旧)
        ...
        if (globalData.loginModel.cardHolder) {
            ...
            GYNetRequest * requset = [[GYNetRequest alloc] initWithBlock:[HSReconsitutionUrl stringByAppendingString:@"/common/qrCodePay"] parameters:@{ @"str" : scranString } requestMethod:GYNetRequestMethodPOST requestSerializer:GYNetRequestSerializerJSON respondBlock:^(NSDictionary *responseObject, NSError *error) {
                ...
                GYOrderPayConfirmVC* orderInfoVc = [[GYOrderPayConfirmVC alloc] init];
