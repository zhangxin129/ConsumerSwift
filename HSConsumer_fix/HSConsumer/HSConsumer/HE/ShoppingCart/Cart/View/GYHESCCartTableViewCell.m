...
- (void)alertTapClick:(UITapGestureRecognizer*)sender
{
    ...
    self.alertView = [[[NSBundle mainBundle] loadNibNamed:@"GYHESCCartAlertView" owner:self options:nil] firstObject];
...
{
    if (!_discountView) {
        _discountView = [[[NSBundle mainBundle] loadNibNamed:@"GYHESCDiscountInfoView" owner:self options:nil] firstObject];
