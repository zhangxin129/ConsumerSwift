...
//    self.invoiceView.hidden = YES;
    ...
    self.leaveMessageTextView.text = kLocalized(@"HE_SC_OrderLeaveMessagePlaceHolder");
...

{
    ...
    if (model.coinIconWidth) {
        self.sendWayLabel.textColor = [UIColor blackColor];
    }
    ...
    if (model.enableApplyCard) {
        ...
        self.applyCardViewHeight.constant = 50.0f;
    ...
    if (kSaftToNSString(model.couponName).length > 0 && kSaftToNSInteger(model.num) > 0) {
        NSString* couponMoney = [NSString stringWithFormat:@"%.2lf", [model.num integerValue] * [model.amount doubleValue]];
...
- (void)textViewDidBeginEditing:(UITextView*)textView
{
    if (textView.tag == 555) {
        ...
        if ([self.leaveMessageTextView.text isEqualToString:kLocalized(@"HE_SC_OrderLeaveMessagePlaceHolder")]) {
