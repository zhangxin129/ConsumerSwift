...
@interface GYEPSaleAfterApplyForOnlyReturnGoodsMoneyViewController () <UITableViewDataSource, UITableViewDelegate, CellForReturnGoodsCellDelegate, UITextViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource, GYNetRequestDelegate,GYGetPhotoViewDelegate>
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint* bgdistanceTop;
...
@property (weak, nonatomic) IBOutlet YYLabel* backMoneyLab;
@property (weak, nonatomic) IBOutlet YYLabel* backAmountLab;
...
@property (weak, nonatomic) IBOutlet UILabel* backMoneyInfoLab;
...
@property (assign, nonatomic) CGFloat itemWH; //cell的高度
@property (assign, nonatomic) CGFloat margin; //cell的间距
...
@implementation GYEPSaleAfterApplyForOnlyReturnGoodsMoneyViewController

- (UITableView*)tableView
{
    if (!_tableView) {
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(_totalPriceView.frame.origin.x, 10, kScreenWidth - 30, ([CellForReturnGoodsCell getHeight] * self.arrDataSource.count - 1)) style:UITableViewStylePlain];
...
- (void)setPropertyWord
{
    ...
    _backMoneyLab.attributedText = text;
    [self setReturnAmount:@"0.00"];
}
...
- (void)setLabelAndButton
{
    ...
    _urlString = @"";
    self.lab.text = kLocalized(@"GYHE_MyHE_UploadDocumentsTips");
    _plorLabel.text = kLocalized(@"GYHE_MyHE_RefundEnter200Characters");
    _plorLabel.textColor = [UIColor lightGrayColor];
    _backMoneyInfoLab.text = kLocalized(@"GYHE_MyHE_RefundInstruction");
    _backMoneyInfoLab.font = [UIFont systemFontOfSize:13];
    _backMoneyInfoLab.textColor = kCorlorFromHexcode(0x8C8C8C);
...
- (void)setHideKeyBoardAndScrollViewFrame
{
    ...
    if (self.arrDataSource.count < 1) {
        WS(weakSelf)
            [GYUtils showMessage:kLocalized(@"GYHE_MyHE_OrderNoRefundGoods") confirm:^{
...
- (void)configCollectionView
{
    ...
    self.diatanceTop.constant = _itemWH - 45;
    layout.itemSize = CGSizeMake(_itemWH, _itemWH);
    layout.minimumInteritemSpacing = 2;
    layout.minimumLineSpacing = _margin;
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(_margin, [CellForReturnGoodsCell getHeight] * self.arrDataSource.count + 250, kScreenWidth - 28, _itemWH * 2 + 12) collectionViewLayout:layout];
...
- (void)collectionView:(UICollectionView*)collectionView didSelectItemAtIndexPath:(NSIndexPath*)indexPath
{
    if (indexPath.row == _selectedPhotos.count) {
        if (indexPath.row < 3) {
            ...
            self.diatanceTop.constant = _itemWH * 2 + 35;
        }
        if (indexPath.row >= 5) {
            return;
        }
...
- (void)setReturnAmount:(NSString*)amount
{
    ...
    _backAmountLab.attributedText = textMoney;
}
...
#pragma mark - tableViewDewlegate
- (NSInteger)tableView:(UITableView*)tableView numberOfRowsInSection:(NSInteger)section
{
    self.bgdistanceTop.constant = 100 + 80 * (self.arrDataSource.count - 1);
