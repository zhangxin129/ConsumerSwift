...
@interface GYEvaluationGoodsViewController () <UITableViewDataSource, UITableViewDelegate, GYNetRequestDelegate>
...
@implementation GYEvaluationGoodsViewController
...
- (void)createBGTip
{
    ...
    titleLabel.text = kLocalized(@"GYHE_MyHE_YouHaveNoCommentRecords");
...
-(void)initView
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"GYEvaluationTableViewCell" bundle:nil] forCellReuseIdentifier:@"cell"];
    ...
    self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    UIView* footer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 14)];
...
- (void)headerReresh
{
    ...
    GYRefreshHeader* header = [GYRefreshHeader headerWithRefreshingBlock:^{
        __strong GYEvaluationGoodsViewController *sself = wself;
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    [cell.btnMakeEvalutaion addTarget:self action:@selector(makeEvalutation:) forControlEvents:UIControlEventTouchUpInside];
    GYEvaluateGoodModel* EvaluateGoodModel = nil;
    if (_marrDataSource.count > indexPath.row) {
        EvaluateGoodModel = _marrDataSource[indexPath.row];
    }
    cell.btnMakeEvalutaion.tag = indexPath.row;
    [cell refreshUIWithModel:EvaluateGoodModel WithType:0];
    return cell;
}
...
- (void)makeEvalutation:(UIButton*)sender
{
    GYEvaluateGoodModel* mod = _marrDataSource[sender.tag];
    GYMakeEvaluationViewController* vcMakeEvaluation = [[GYMakeEvaluationViewController alloc] initWithNibName:@"GYMakeEvaluationViewController" bundle:nil];
    vcMakeEvaluation.model = mod;
    //add by zhangqy 9295 iOS消费者--我的评价--待评价列表中商品评价提交跳转回列表后，此条评价还保留在待评价列表，需刷新才消失
    vcMakeEvaluation.refreshListBlock = ^(GYEvaluateGoodModel *emodel){
        [_marrDataSource removeObject:emodel];
        [self.tableView reloadData];
    };
    [self.nav pushViewController:vcMakeEvaluation animated:YES];
}
