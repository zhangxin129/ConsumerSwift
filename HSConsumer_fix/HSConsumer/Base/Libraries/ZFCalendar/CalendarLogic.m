...
- (NSMutableArray*)reloadCalendarView:(NSDate*)date selectDate:(NSDate*)selectdate needDays:(int)days_number andClickdates:(NSArray*)noclick
{
    ...
    for (int i = 0; i <= months; i++) {
        ...
        [self calculateDaysInCurrentMonthWithDate:month andArray:calendarDays andNoclickdate:noclick andselectDate:select];
        [self calculateDaysInFollowingMonthWithDate:month andArray:calendarDays andnoclick:noclick]; //计算下月份的天数
...
- (void)calculateDaysInFollowingMonthWithDate:(NSDate*)date andArray:(NSMutableArray*)array andnoclick:(NSArray*)noclick
{
    ...
    for (int i = 1; i < partialDaysCount + 1; ++i) {
        ...
        //        [self changStyle:calendarDay andNoClick:noclick];
        [array addObject:calendarDay];
    }
}
...
- (void)calculateDaysInCurrentMonthWithDate:(NSDate*)date andArray:(NSMutableArray*)array andNoclickdate:(NSArray*)noclickdates andselectDate:(NSDate*)selectDate
{

    NSUInteger daysCount = [date numberOfDaysInCurrentMonth]; //计算这个月有多少天
    NSDateComponents* components = [date YMDComponents]; //今天日期的年月日
    //    NSDateComponents *dtcomp  = [date YMDComponents];
    NSDateFormatter* noclick = [[NSDateFormatter alloc] init];
    [noclick setDateFormat:@"yyyy-MM-dd"];
    ...
    for (int i = 1; i < daysCount + 1; ++i) {
        ...
        //        for (int t = 0; t<noclickdates.count; t++) {
        //            NSDate *dt = [noclick dateFromString:noclickdates[t]];
        //            dtcomp = [dt YMDComponents];
        //            if (dtcomp.year == calendarDay.year & dtcomp.month == calendarDay.month & dtcomp.day==calendarDay.day) {
        //                [self changStyle:calendarDay andNoClick:dtcomp];
        //            }else
        //            {
        [self changStyle:calendarDay andNoClick:noclickdates];
        //            }
        //        }
...
- (void)changStyle:(CalendarDayModel*)calendarDay andNoClick:(NSArray*)noclickdate
{
    NSDateComponents* calendarToDayNew = [[NSDate date] YMDComponents]; ///今天
    NSDateComponents* calendarToDay = [today YMDComponents]; //开始时间
    ...
    for (int i = 0; i < noclickdate.count; i++) {
        NSDate* noclick = [formatr dateFromString:noclickdate[i]];
        NSDateComponents* noclickda = [noclick YMDComponents]; //默认选择的那一天
        if (noclickda.year == calendarDay.year & noclickda.month == calendarDay.month & noclickda.day == calendarDay.day) {
    ...
    if (calendarSelect.year == calendarDay.year & calendarSelect.month == calendarDay.month & calendarSelect.day == calendarDay.day) {
        ...
        //昨天乃至过去的时间设置一个灰度
        if (calendarToDay.year >= calendarDay.year & calendarToDay.month >= calendarDay.month & calendarToDay.day > calendarDay.day) {

            calendarDay.style = CellDayTypePast;

            //之后的时间时间段
        }
        else if (calendarbefore.year <= calendarDay.year & calendarbefore.month <= calendarDay.month & calendarbefore.day <= calendarDay.day) {
    ...
    else if (calendarToDay.year == calendarDay.year && calendarToDay.month == calendarDay.month && calendarToDay.day - calendarDay.day == -1) {
        //        calendarDay.holiday = @"明天";

        //后天
    }
    else if (calendarToDay.year == calendarDay.year && calendarToDay.month == calendarDay.month && calendarToDay.day - calendarDay.day == -2) {
        //        calendarDay.holiday = @"后天";

        //1.1元旦
    }
    else if (calendarDay.month == 1 && calendarDay.day == 1) {
    ...
    else if (calendarDay.month == 11 && calendarDay.day == 1) {
