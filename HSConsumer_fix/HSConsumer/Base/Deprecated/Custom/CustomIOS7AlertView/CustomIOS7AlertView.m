...
{
    DDLogDebug(@"Button Clicked! %d, %d", (int)buttonIndex, (int)[alertView tag]);
    [self close];
}
...
- (UIView*)createContainerView
{
    ...
    gradient.colors = [NSArray arrayWithObjects:
                                   (id)[[UIColor colorWithRed:255 / 255.0f green:245 / 255.0f blue:225 / 255.0f alpha:1.0] CGColor],
                               (id)[[UIColor colorWithRed:255 / 255.0f green:245 / 255.0f blue:225 / 255.0f alpha:1.0] CGColor],
                               (id)[[UIColor colorWithRed:255 / 255.0f green:245 / 255.0f blue:225 / 255.0f alpha:1.0] CGColor],
                               nil];
    ...
    self.lineView1 = [[UIImageView alloc] initWithFrame:CGRectMake(20, containerView.bounds.origin.y + 44, dialogContainer.bounds.size.width - 40, buttonSpacerHeight)];

    self.lineView1.image = [UIImage imageNamed:@"gyhe_line_confirm_dialog_yellow"];
    [dialogContainer addSubview:self.lineView1];

    // 设置 下分隔线＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    self.lineView = [[UIImageView alloc] initWithFrame:CGRectMake(20, dialogContainer.bounds.size.height - buttonHeight - buttonSpacerHeight - 10, dialogContainer.bounds.size.width - 40, buttonSpacerHeight)];
    self.lineView.image = [UIImage imageNamed:@"gyhe_line_confirm_dialog_yellow"];
...
- (void)addButtonsToView:(UIView*)container
{
    ...
    for (int i = 0; i < [buttonTitles count]; i++) {
        ...
        [closeButton setFrame:CGRectMake(i * buttonWidth + 20, container.bounds.size.height - buttonHeight + 5, buttonWidth - 40, buttonHeight - 20)];
        ...
        //        closeButton.layer.borderColor = [UIColor colorWithRed:239/255.0f green:65/255.0f blue:54/255.0f alpha:1.0].CGColor;
        //        [closeButton setBackgroundColor:[UIColor colorWithRed:239/255.0f green:65/255.0f blue:54/255.0f alpha:1.0]];
        if (self.arrBtnBackColor && self.arrBtnBackColor.count > 0) {
            if (self.arrIndesx.count && self.arrIndesx.count > 0) {
                for (NSString* strIndex in self.arrIndesx) {
                    if ([strIndex integerValue] == i) {
                        UIColor* color = [self.arrBtnBackColor objectAtIndex:i];
                        [closeButton setBackgroundColor:color];
                    }
                    else {
                        [closeButton setBackgroundImage:[UIImage imageNamed:@"gyhe_alert_btn_confirm_bg"] forState:UIControlStateNormal];
                    }
                }
            }
        }
