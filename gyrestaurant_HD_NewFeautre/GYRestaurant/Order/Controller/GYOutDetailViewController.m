...
@interface GYOutDetailViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@property (nonatomic, strong) UIButton *selectPostBtn;
//用户名
...
@implementation GYOutDetailViewController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
-(void)creatUI{
   self.navigationItem.leftBarButtonItem = [Utils createBackButtonWithTitle:kLocalized(@"Orderdetails") withTarget:self withAction:@selector(popBack)];
    ...
    UILabel *ordTypeTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(720 + 4*x, 5, 40, 40)];
    ordTypeTitleLabel.text = kLocalizedAddParams(kLocalized(@"Types"), @":");
    [titleView addSubview:ordTypeTitleLabel];
    
    _ordTypeLabel = [[UILabel alloc]initWithFrame:CGRectMake(760 + 4*x, 5, 40, 40)];
   // _ordTypeLabel.text = @"店内";
    [titleView addSubview:_ordTypeLabel];
    
    UILabel *ordStatusTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(800 + 5*x, 5, 40, 40)];
    ordStatusTitleLabel.text = kLocalizedAddParams(kLocalized(@"Status"), @":");
    [titleView addSubview:ordStatusTitleLabel];
    
    _ordStatusLabel = [[UILabel alloc]initWithFrame:CGRectMake(840 + 5*x, 5, 120, 40)];
...
- (void)refreshUI
{
   ...
   // _remarLable.text = detailModel.deliDiscount[];
    ...
    _totalPayLable.adjustsFontSizeToFitWidth = YES ;
    _pointLable.text = detailModel.totalPv;
    _pointLable.adjustsFontSizeToFitWidth = YES;
