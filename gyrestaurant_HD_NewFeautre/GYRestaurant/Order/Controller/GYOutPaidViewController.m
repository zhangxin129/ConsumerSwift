...
@interface GYOutPaidViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation GYOutPaidViewController
...
//    [ordDetailView addSubview:_nameLable];
   ...
   // _addressLable.text = @"深圳市南山区学府路百度国际大厦205";
    ...
    //送达时间
    UILabel *arTimeLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 80, 80, 30)];
    
    arTimeLabel.text = kLocalizedAddParams(kLocalized(@"ArrivalsTime"), @":");
    [ordDetailView addSubview:arTimeLabel];
    
    _arriveTimeLable = [[UILabel alloc]initWithFrame:CGRectMake(90, 80, 180, 30)];
    _arriveTimeLable.textAlignment = NSTextAlignmentLeft;
   ...
   // _remarkLabel.text = @"少辣，准备一个儿童凳子";
    ...
    _payBtn.frame = CGRectMake(550, 5, 100, 40);
    _payBtn.backgroundColor = kRedFontColor;
    [_payBtn.layer setCornerRadius:15.0];
    [_payBtn setTitle:kLocalized(@"Receivables") forState:UIControlStateNormal];
    [_payBtn addTarget:self action:@selector(receivables:) forControlEvents:UIControlEventTouchUpInside];
    _payBtn.titleLabel.textColor = kBlueFontColor;
    _payBtn.titleLabel.font = [UIFont systemFontOfSize:25];
    [downView addSubview:_payBtn];
...
-(void)receivables:(UIButton *)sender{
    ...
    alert.rightBlock = ^(){
        ...
        [viewModel outOrderPayWithkey:globalData.loginModel.token userId:_infoDic[@"userId"] orderId:_infoDic[@"orderId"] orderType:@"2"];
