...
@interface GYOutConfirmViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation GYOutConfirmViewController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
//    [ordDetailView addSubview:_nameLable];
   ...
   // _remarkLabel.text = @"少辣，准备一个儿童凳子";
    ...
    [sureBtn setTitle:kLocalized(@"TakeOrders") forState:UIControlStateNormal];
    ...
    refuseBtn.frame = CGRectMake(670, 5, 100, 40);
    refuseBtn.backgroundColor = kRedFontColor;
    [refuseBtn.layer setCornerRadius:15.0];
    [refuseBtn setTitle:kLocalized(@"RefuseOrders") forState:UIControlStateNormal];
    [refuseBtn addTarget:self action:@selector(refuse:) forControlEvents:UIControlEventTouchUpInside];
    refuseBtn.titleLabel.textColor = kBlueFontColor;
    refuseBtn.titleLabel.font = [UIFont systemFontOfSize:25];
    [downView addSubview:refuseBtn];
...
-(void)confirm:(UIButton *)sender{
    ...
    alertView.rightBlock=^(){
        ...
        [self modelRequestNetwork:viewModel :^(id resultDic) {
            DDLogCInfo(kLocalized(@"OrdersSuccess"));
        ...
        [viewModel acceptOrderWithuserId:_infoDic[@"userId"]  withOrderId:_infoDic[@"orderId"]  withIsAccept:YES];
    };
...
-(void)refuse:(UIButton *)sender{
    ...
    __block UIButton *dBtn = sender;
    GYReasonAlertView *alertView = [[GYReasonAlertView alloc] initWithTitle:kLocalizedAddParams(kLocalized(@"AreYouSureToRefuseOrders"), @"?") resonTextFieldName:@"拒接理由:" leftButtonTitle:kLocalized(@"Cancel") rightButtonTitle:kLocalized(@"Determine")];
    
    alertView.rightBlock= ^(NSString *reasonStr){
       
       
        if (reasonStr.length > 0 && reasonStr.length < 20) {
            ...
            [self modelRequestNetwork:viewModel :^(id resultDic) {
                ...
                DDLogCInfo(kLocalized(@"RefuseOrdersSuccess"));
            ...
            [viewModel putRefuseOrderWithUserId:_infoDic[@"userId"] withOrderId:_infoDic[@"orderId"] withReason:reasonStr];
