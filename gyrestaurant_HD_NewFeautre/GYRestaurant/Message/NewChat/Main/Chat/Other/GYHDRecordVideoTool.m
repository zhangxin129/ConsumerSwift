...
//  by jianglincen 创建时实时获取设备方向，赋值给图层
...
- (void)startRecordVideo
{
 ...
 //   DDLogCInfo(@"captureConnection.videoOrientation=%d layer.orientation=%d",captureConnection.videoOrientation,[self.captureVideoPreviewLayer connection].videoOrientation);
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    //test 方法如下: by jianglincen
   // [self mergeAndexportVideoAtFileURL:outputFileURL];
    //保存到相册视频播放不倒转 jianglincen
    /*
...
// 旋转播放方向,by jianglincen
    
//    if (!([self videoOrientationFromCurrentDeviceOrientation]==AVCaptureVideoOrientationLandscapeRight)) {
...
//    }
//    
//    DDLogCInfo(@"rotation .degress =%d",degress);
