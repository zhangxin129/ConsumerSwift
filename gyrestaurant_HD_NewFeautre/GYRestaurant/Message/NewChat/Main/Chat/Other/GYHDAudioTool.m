...
/**录音时长*/
static const NSTimeInterval timelen= 29.0f;//设置29秒来确保时间显示正常
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        DDLogCInfo(@"%f",self.recorder.deviceCurrentTime - self.deviceCurrentTime);
...
- (void)stopRecord
{
    if ([self.recorder isRecording]) {
        ...
        self.mp3Data = [NSData dataWithContentsOfFile:self.mp3Path];
        self.duration = self.recorder.deviceCurrentTime - self.deviceCurrentTime;
        DDLogCInfo(@"%f",self.recorder.currentTime);
    }
}
...
- (void)audio_PCMUrl:(NSString *)cafFilePath toMP3:(NSString *)mp3FilePath
{
    ...
    @catch (NSException *exception) {
        DDLogCInfo(@"%@",[exception description]);
    }
}
@end
