...
- (void)setChatModel:(GYHDNewChatModel *)chatModel
{
    ...
    NSDictionary *videoDict = [Utils stringToDictionary:self.chatModel.chatDataString];
    ...
    if (image) {
        ...
        [self.chatPictureImageView setImageWithURL:[NSURL URLWithString:bodyDict[@"msg_imageNailsUrl"]] placeholder:[UIImage imageNamed:@"ep_placeholder_image_type1"] options:kNilOptions completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
            CALayer *layer = ImageView.layer;
            layer.frame = (CGRect){{0,0},ImageView.layer.frame.size};
            self.chatPictureImageView.layer.mask = layer;
        }];
...
- (void)setupAuto
{
    ...
    [self.chatStateButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(weakSelf.chatPictureImageView.mas_left);
